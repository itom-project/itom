...
class UiShapeDemo(ItomUi):
    ...
    def startInteraction(self, type):
        ...
        if not modal:
            ...
            try:
                ...
                shapes = plotItem(self.gui.plot).drawAndPickElements(
                    type, nrOfElements
                )
                ...
                self.gui.listLog.call(
                    "addItem", "Modal interaction interrupted"
                )
    ...
    def on_btnCreateAndShowMask_clicked(self):
        mask = self.gui.plot["source"].createMask(
            self.gui.plot["geometricShapes"]
        )
        plot(mask)
    ...
    def on_plot_geometricShapeChanged(self, idx, shape):
        self.gui.listLog.call(
            "addItem", "shape %i changed: " % idx + str(shape)
        )

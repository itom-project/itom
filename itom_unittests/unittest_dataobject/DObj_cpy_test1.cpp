...
/*! \class copyTests1
    ...
    This test class checks functionality of deepCopyPartial(..) method on different data objects of different sizes and
   types.
*/
...
/*!
    ...
    Also values outside of ROI of destination must remain equal if checked after adjusting ROI back to original size and
   position. The above two conditions are checked in this test for 2 and 3 dimensional data objects.
*/
TYPED_TEST(copyTests1, deepCopyPartial_Test2)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            EXPECT_EQ(this->dObj2_s.template at<TypeParam>(i, j), this->dObj2_d.template at<TypeParam>(i, j));
        } //!< assigning unique value to each element of Data Object this->dObj2_s.
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            ...
            {
                EXPECT_EQ(
                    this->dObj3_s.template at<TypeParam>(i, j, k),
                    this->dObj3_d.template at<TypeParam>(
                        i, j,
                        k)); //!< assigning unique value to each element of 3 dimensional Data Object this->dObj3_s
            }
        }
    }
    ...
    {
        for (int j = 0; j < 5; j++)
            EXPECT_EQ(this->dObj2_sr.template at<TypeParam>(i, j),
                      this->dObj2_dr.template at<TypeParam>(
                          i, j)); //!< Check if the values of elements in ROI are same as in the original
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            for (int k = 0; k < 2; k++)
            {
                EXPECT_EQ(this->dObj3_sr.template at<TypeParam>(i, j, k),
                          this->dObj3_dr.template at<TypeParam>(
                              i, j, k)); //!< Checking if elements of ROI of original Data Object this->dObj3_sr has
    ...
    {
        for (int j = 0; j < 10; j++)
            EXPECT_EQ(this->dObj2_s.template at<TypeParam>(i, j),
                      this->dObj2_dr.template at<TypeParam>(
                          i, j)); //!< Check if the values of elements in ROI are same as in the original
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            for (int k = 0; k < 5; k++)
            {
                EXPECT_EQ(this->dObj3_s.template at<TypeParam>(i, j, k),
                          this->dObj3_dr.template at<TypeParam>(
                              i, j, k)); //!< Checking if elements of ROI of original Data Object this->dObj3_sr has
                                         //!< been changed while copying it to this->dObj3_dr.
            }
...
TYPED_TEST(copyTests1, deepCopyPartial_Test3)
{
    int planeId;
    TypeParam *rowPtr =
        NULL; //!< Row pointer to locate each element of 5 dimensional data object this->dObj4_s at each row
    int planeId_d;
    TypeParam *rowPtr_d =
        NULL; //!< Row pointer to locate each element of 5 dimensional data object this->dObj4_d at each row
        ...
        for (int j = 0; j < dim2; j++)
        {
            for (int k = 0; k < dim3; k++)
            {
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    {
                        rowPtr[m] = cv::saturate_cast<TypeParam>(
                            i * 10000 + j * 1000 + k * 100 + l * 10 +
                            m); //!< Assigning unique value to each element of this->dObj4_s
                    }
                }
            }
        }
        ...
        for (int j = 0; j < dim2_d; j++)
        {
            for (int k = 0; k < dim3_d; k++)
            {
                ...
                for (int l = 0; l < dim4_d; l++)
                {
                    ...
                    {
                        EXPECT_EQ(rowPtr_d[m], cv::saturate_cast<TypeParam>(
                                                   i * 10000 + j * 1000 + k * 100 + l * 10 +
                                                   m)); //!< checking if the values of elements are same as original
                                                        //!< data object after applying deepCopyPartial() function.
                    }
                }
            }
        }
...
/*!
    In this test, we define a 5 dimension data object, make a deep copy from it using deepCopyPartial() function ,
   adjust the ROI of copied data object to smaller ROI, adjust the ROI back to original size of source data object and
   then compare the values of the elements if the values have been changed.
*/
            ...
            for (int k = 0; k < dim3; k++)
            {
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    for (int m = 0; m < dim5; m++)
                    {
                        rowPtr[m] = cv::saturate_cast<TypeParam>(
                            i * 10000 + j * 1000 + k * 100 + l * 10 +
                            m); //!< Assigning unique value to each element of data object this->dObj4_s
                    }
                }
            }
        }
            ...
            for (int k = 0; k < dim3_d; k++)
            {
                ...
                for (int l = 0; l < dim4_d; l++)
                {
                    ...
                    for (int m = 0; m < dim5_d; m++)
                    {
                        EXPECT_EQ(rowPtr_d[m], cv::saturate_cast<TypeParam>(
                                                   i * 10000 + j * 1000 + k * 100 + l * 10 +
                                                   m)); //!< checking the element values of this->dObj4_dr are same as
                                                        //!< the original data object after adjusting ROI back to
                                                        //!< original size same as original data object this->dObj4_s
                    }
                }
            }
        }
...
/*!
    This test checks that it is possible to copy the content of a 1x5 array to a 5x1x1 array (same type and same size if
   all one-dimensions are omitted)

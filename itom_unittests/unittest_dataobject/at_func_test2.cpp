...
// at_Test4
/*!
    This test checks the functionality of at(ito::Range *ranges) function with 5 dimensional data objects using full
   ranges as parameter.
*/
TYPED_TEST(at_func_test2, at_Test4)
{
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    for (int m = 0; m < dim5; m++)
                    {
                        rowPtr1[m] = cv::saturate_cast<TypeParam>(this->calcUniqueValue5D(
                            i, j, k, l, m)); //!< Assigning unique value to each element of dObj4.
                    }
                }
            }
        }
    ...
    dObj4_test1 = this->dObj4.at(ranges1); //!< assigning the full data object dObj4 to dObj4_test1 using full range
                                           //!< array defined with ranges1[].
        ...
        for (int j = 0; j < dim2; j++)
        {
            ...
            for (int k = 0; k < dim3; k++)
            {
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    {
                        idx[4] = m;
                        EXPECT_EQ(this->dObj4.template at<TypeParam>(idx),
                                  dObj4_test1.at<TypeParam>(idx)); //!< Checking values of  5 dimensional data object
                                                                   //!< dObj4 after applying adjustROI().
                    }
                }
            }
        }
...
// at_Test5
/*!
    This test checks the functionality of at(ito::Range *ranges) function on 5 dimensional data object dObj4 using
   limited range as parameter.
*/
TYPED_TEST(at_func_test2, at_Test5)
{
    ...
    */
    ...
    ranges1[4].start = 2;
    ranges1[4].end = 3;
    int test_res5d[] = {12,  22,  112, 122, 212,
                        222, 312, 322, 412, 422}; //!< Expected result vector after applying at() function on dObj4.
    dObj4_test1 = this->dObj4.at(ranges1); //!< assigning the full data object dObj4 to dObj4_test1 using limited range
                                           //!< array defined with ranges1[].
            ...
            for (int k = 0; k < dim3; k++)
            {
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    for (int m = 0; m < dim5; m++)
                    {
                        ...
                        EXPECT_EQ(
                            v1, v2); //!< Checking values of 5 dimensional data object dObj4 after applying adjustROI().
                    }
                }
            }
        }
...
// at_Test6
/*!
    This test checks the functionality of at(ito::Range *ranges) function on 5 dimensional data object dObj4 using wrong
   range limits as parameter.
*/
TYPED_TEST(at_func_test2, at_Test6)
{
    ...
    /*!
    Assigning limits in ranges1[] for at() function to be applied on 4 dimensional data object dObj5. These ranges
    limits fall outside of size of dObj4 which should raise exception while test.
    */
    ito::DataObject dObj4_test1;
    ito::Range ranges1[5];
    ranges1[0].start = 0;

...
TYPED_TEST(makeContinuousFunc_test, nonContTest_3d)
{
    ...
    int test_res3d[] = {36, 37, 38, 41, 42, 43,
                        61, 62, 63, 66, 67, 68};  //!< Expected result vector for dObj3 after adjustROI method using 2
    ...
    for (int i = 0; i < dim1; i++)
    {
        for (int j = 0; j < dim2; j++)
        {
            ...
            {
                this->dObj_3d.template at<TypeParam>(i, j, k) = cv::saturate_cast<TypeParam>(
                    temp++); //!< Assigning each element of the data object this->dObj_3d with a unique integer value.
            }
        }
    }

    this->dObj_3d.adjustROI(
        3, matLimits3d); //!< adjusting ROI of dObj3 with general 3 parameter adjustROI method to desired position
    ...
    for (int i = 0; i < dim1; i++)
    {
        ...
        for (int j = 0; j < dim2; j++)
        {
            ...
            for (int k = 0; k < dim3; k++)
            {
                ...
                EXPECT_EQ(
                    this->dObj_3dres.template at<TypeParam>(i, j, k),
                    cv::saturate_cast<TypeParam>(
                        test_res3d[temp++])); //!< Testing each element value of resulted data object this->dObj_3dres
                                              //!< with respective element value of source data object this->dObj_3d
                                              //!< after being applied to makeContinuous() function.
            }
        }
    }
}
...
TYPED_TEST(makeContinuousFunc_test, nonContTest_4d)
{
    ...
    int matLimits4d[] = {-1, -1, 0,  -1,
                         0,  0,  -1, -1}; //!< defining ROI offsets for 4 Dimensional Data Object this->dObj_4d
    int test_res4d[] = {31, 34, 37, 40, 43, 46, 49, 52, 61,
                        64, 67, 70, 73, 76, 79, 82}; //!< Expected result vector for dObj3 after adjustROI method using
                                                     //!< 2 parameter (general) implementation
    int temp = 0;
    ...
    for (int j = 0; j < dim1; j++)
    {
        for (int k = 0; k < dim2; k++)
        {
            ...
            for (int l = 0; l < dim3; l++)
            {
                ...
                {
                    rowPtr1[m] = cv::saturate_cast<TypeParam>(
                        temp++); //!< assign unique value to each element of data object dObj4
                }
            }
        }
    }
    // std::cout << this->dObj_4d << std::endl;
    this->dObj_4d.adjustROI(
        4,
        ...
        for (int j = 0; j < dim2; j++)
        {
            ...
            for (int k = 0; k < dim3; k++)
            {
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    EXPECT_EQ(v1, v2); //!< Testing each element value of resulted data object this->dObj_4dres with
                                       //!< respective element value of source data object this->dObj_4d after being
                                       //!< applied to makeContinuous() function.
                }
            }
        }
    }
...
TYPED_TEST(makeContinuousFunc_test, ContTest_3d)
{
    ...
    int test_res3d[] = {32, 33, 37, 38, 42, 43,
                        57, 58, 62, 63, 67, 68}; //!< Expected result vector for dObj3 after adjustROI method using 2
                                                 //!< parameter (general) implementation
    ...
    for (int i = 0; i < dim1; i++)
    {
        for (int j = 0; j < dim2; j++)
        {
            for (int k = 0; k < dim3; k++)
            {
                dObj_3d_con.at<TypeParam>(i, j, k) = cv::saturate_cast<TypeParam>(
                    temp++); //!< Assigning each element of the data object this->dObj_3d with a unique integer value.
            }
        }
    }

    dObj_3d_con.adjustROI(
        3, matLimits3d); //!< adjusting ROI of dObj3 with general 3 parameter adjustROI method to desired position
    ...
    //        for(int k=0; k<2; k++)
    //        {
    //            EXPECT_EQ(dObj_3d_con.at<TypeParam>(i,j,k),cv::saturate_cast<TypeParam>(test_res3d[temp++]));    //!<
    //            Testing each element value of resulted data object this->dObj_3dres with respective element value of
    //            source data object this->dObj_3d after being applied to makeContinuous() function.
    //        }
    //    }
    //}
}

...
{
    TypeParam tempVal = this->valDarkGray * this->valBLACKTransparent; // Multiplication off zero with white must be zeros;
...
TYPED_TEST(rgbaOperatorTests, testConstructors)
{
    if(typeid(TypeParam) == typeid(ito::Rgba32))
    {
        ...
        EXPECT_EQ((*static_cast<ito::RGBChannel<ito::Rgba32::RGBA_R>*>(&c1)).value(), ito::Rgba32(255,255,0,0).red());

        EXPECT_EQ(ito::Rgba32(255, 0, 255,0), ito::Rgba32(255,0,255,0));
        EXPECT_NE(ito::Rgba32(255, 12, 255,0), ito::Rgba32(255,0,255,0));
        EXPECT_EQ((*static_cast<ito::RGBChannel<ito::Rgba32::RGBA_G>*>(&c2)).value(), ito::Rgba32(255,0,255,0).green());

        EXPECT_EQ(ito::Rgba32(255, 0, 0,255), ito::Rgba32(255,0,0,255));
        EXPECT_NE(ito::Rgba32(255, 12, 255,0), ito::Rgba32(255,0,0,255));
        EXPECT_EQ((*static_cast<ito::RGBChannel<ito::Rgba32::RGBA_B>*>(&c3)).value(), ito::Rgba32(255,0,0,255).blue());
...
TYPED_TEST(rgbaOperatorTests, testGrayConversion)
{
    ...
    EXPECT_TRUE( ito::isZeroValue<ito::float32>(this->valWhite.gray()-255.0, std::numeric_limits<ito::float32>::epsilon()));
    EXPECT_TRUE( ito::isZeroValue<ito::float32>(this->valBLACKTransparent.gray(), std::numeric_limits<ito::float32>::epsilon()));
}

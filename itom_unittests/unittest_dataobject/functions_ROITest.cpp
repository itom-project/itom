...
TYPED_TEST(ROITest, adjustROITest2d)
{
    ...
    int orgSize2d[] = {0,0};
    int offsets2d[] = {0,0};
    unsigned int res1_str[] = {53,54,55,56,57,58,59,60,66,67,68,69,70,71,72,73,79,80,81,82,83,84,85,86,92,93,94,95,96,97,98,99}; // expected result array for testing elements of shrunk matrix this->mat1_2d after adjustROI
    ...
    for(int i =0;i<12;i++) 
        {
            for(int j=0;j<13;j++)
            {this->mat1_2d.template at<TypeParam>(i,j) = cv::saturate_cast<TypeParam>(13*i+j);}   //assigning unique value to each element of this->mat1_2d
        }

    this->mat1_2d.adjustROI(matDims2d,matLimits2d);   //adjust ROI of this->mat1_2d (shrinking because offset values are negative)
    this->mat1_2d.locateROI(orgSize2d,offsets2d);        //retrieve offset values of this->mat1_2d

    for(int i=0; i<this->mat1_2d.getDims();i++) EXPECT_EQ(orgSize2d[i],expSize2d[i]);  //check retrieved size of matrix matches with original
    for(int i=0; i<this->mat1_2d.getDims();i++) EXPECT_EQ(offsets2d[i],expOffsets2d[i]);  //check retrieved offsets of ROI of this->mat1_2d matches with original

    for(int i =0;i<4;i++) 
        {
            for(int j=0;j<8;j++)EXPECT_EQ(this->mat1_2d.template at<TypeParam>(i,j),cv::saturate_cast<TypeParam>(res1_str[i*8+j])); //check if the values of elements changed from the original
        }

    this->mat1_2d.adjustROI(4,4,1,4);   //again expanding this->mat1_2d to original size
    this->mat1_2d.locateROI(orgSize2d,offsets2d);   //retrieve the dimensions and offset of ROI for expanded this->mat1_2d
    for(int i =0;i<2;i++) expOffsets2d[i] = 0;

    for(int i=0; i<this->mat1_2d.getDims();i++) EXPECT_EQ(orgSize2d[i],expSize2d[i]);  //check retrieved size of the matrix matches with original
    for(int i=0; i<this->mat1_2d.getDims();i++) EXPECT_EQ(offsets2d[i],expOffsets2d[i]); //check if the offsets of this->mat1_2d are 0 after expanding the matrix to its original size
    
    for(int i =0;i<12;i++) 
        {
            for(int j=0;j<13;j++)
            {EXPECT_EQ(this->mat1_2d.template at<TypeParam>(i,j),cv::saturate_cast<TypeParam>(13*i+j));}  //check if the values of elements remained same as defined earlier after expanding it to original size
        }
...
TYPED_TEST(ROITest, adjustROITest3d)
{
    ...
    for(int i=0; i<this->mat1_3d.getDims();i++) EXPECT_EQ(orgSize3d[i],expSize3d[i]);        //checks if the size of matrix still sustain its original value as expected after adjusting ROI to desired location
    for(int i=0; i<this->mat1_3d.getDims();i++) EXPECT_EQ(offsets3d[i],expOffsets3d[i]);    //cheks if retrieved offset values for ROI after adjusting it are same as expected
    ...
    for(int i =0;i<4;i++) 
    {
        ...
        {  
            for(int k=0;k<5;k++)
            {
                this->mat2_3d.template at<TypeParam>(i,j,k) = cv::saturate_cast<TypeParam>(temp++);   //assigning unique value to each element of 3 dimensional matrix this->mat2_3d
            }
        }
    }
    ...
    for(int i =0;i<2;i++) 
    {
        ...
        {  
            for(int k=0;k<3;k++)
            {
                EXPECT_EQ(this->mat2_3d.template at<TypeParam>(i,j,k),cv::saturate_cast<TypeParam>(res2_str[temp++]));   //checking value to each element of this->mat2_3d
            }
        }
    }
    ...
    for(int i =0;i<3;i++) 
    {
        ...
        {  
            for(int k=0;k<4;k++)
            {
                EXPECT_EQ(this->mat2_3d.template at<TypeParam>(i,j,k),cv::saturate_cast<TypeParam>(res3_str[temp++]));   //checking value to each element of this->mat2_3d
            }
        }
    }
...
TYPED_TEST(ROITest, checkSizeTest)
{
    ...
    EXPECT_TRUE(this->mat1_2d.at(ito::Range(1,5), ito::Range(2,4)).getSize() == this->mat2_2d.at(ito::Range(2,6), ito::Range(1,3)).getSize());
    EXPECT_FALSE(this->mat1_2d.at(ito::Range(1,5), ito::Range(2,4)).getSize() != this->mat2_2d.at(ito::Range(2,6), ito::Range(1,3)).getSize());

...
{
    int check_arry1[] = {0,  1,  2,  10, 11, 12,
                         20, 21, 22, 30, 31, 32}; //!< expected result array after applying at() function to dObj2

    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            this->dObj2.template at<TypeParam>(i, j) =
                cv::saturate_cast<TypeParam>(10 * i + j); //!< Assigning unique value to each element of dObj2.
        }
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            EXPECT_EQ(
                dObj2_test.template at<TypeParam>(i, j),
                check_arry1[temp++]); //!< checks if the element values of dObj2_test, obtained with at() function
                                      //!< applied on data object dObj2, is same as the original data object dObj2.
        }
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            for (int k = 0; k < 5; k++)
            {
                this->dObj3.template at<TypeParam>(i, j, k) =
                    cv::saturate_cast<TypeParam>(temp++); //!< Assigning unique value to each element of dObj3.
            }
        }
    }
    ...
    ranges_3d[2].start = 0;
    ranges_3d[2].end = 3;
    int check_arry2[] = {25, 26, 27, 30, 31, 32,
                         50, 51, 52, 55, 56, 57}; //!< expected result array after applying at() function to dObj3
    ...
    for (int i = 0; i < 2; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            for (int k = 0; k < 3; k++)
            {
                EXPECT_EQ(dObj3_test.at<TypeParam>(i, j, k),
                          cv::saturate_cast<TypeParam>(
                              check_arry2[temp++])); //!< checks if the element values of dObj3_test, obtained with at()
                                                     //!< function applied on data object dObj3, is same as the original
                                                     //!< data object dObj3.
            }
        }
    }
...
TYPED_TEST(at_func_test1, at_Test2)
{
    ...
    dObj2_test1 = this->dObj2.at(
        ranges1); //!< assigning the full data object dObj2 to dObj2_test1 using full range array defined with range1[].

    EXPECT_EQ(this->dObj2.getDims(),
              dObj2_test1.getDims()); //!< dimension of both data objects should be same after applying at() function
    EXPECT_EQ(this->dObj2.getSize(0),
              dObj2_test1.getSize(0)); //!< checking if the size of 0th dimension is same as the original
    EXPECT_EQ(this->dObj2.getSize(1),
              dObj2_test1.getSize(1)); //!< checking if the size of 1st dimension is same as the original
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            EXPECT_EQ(dObj2_test1.at<TypeParam>(i, j),
                      cv::saturate_cast<TypeParam>(
                          10 * i + j)); //!< checks if the element values of dObj2_test1, obtained with at() function
                                        //!< applied on data object dObj2, is same as the original data object dObj2.
        }
    }
    ...
    */
    ...
    ranges_3d[1] = ito::Range::all(); //!< ranges_3d[1] element should point to full range of 1st dimension of the
                                      //!< related data object.
    ranges_3d[2] = ito::Range::all(); //!< ranges_3d[2] element should point to full range of 2nd dimension of the
                                      //!< related data object.
    dObj3_test1 = this->dObj3.at(ranges_3d); //!< assigning the full data object dObj3 to dObj3_test1 using full range
                                             //!< array defined with range_3d[].

    EXPECT_EQ(this->dObj3.getDims(),
              dObj3_test1.getDims()); //!< dimension of both data objects should be same after applying at() function
    EXPECT_EQ(this->dObj3.getSize(0),
              dObj3_test1.getSize(0)); //!< checking if the size of 0th dimension is same as the original
    EXPECT_EQ(this->dObj3.getSize(1),
              dObj3_test1.getSize(1)); //!< checking if the size of 1st dimension is same as the original
    EXPECT_EQ(this->dObj3.getSize(2),
              dObj3_test1.getSize(2)); //!< checking if the size of 2nd dimension is same as the original
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            for (int k = 0; k < 5; k++)
            {
                EXPECT_EQ(dObj3_test1.at<TypeParam>(i, j, k),
                          cv::saturate_cast<TypeParam>(
                              temp++)); //!< checks if the element values of dObj3_test1, obtained with at() function
                                        //!< applied on data object dObj3, is same as the original data object dObj3.
            }
        }
    }
}
...
TYPED_TEST(at_func_test1, at_Test3)
{
    ...
    //!< Test code with full ranges
    ito::DataObject dObj2_test1;
    ito::Range ranges1[2]; //!< Range variable to be used as parameter with at() function. In this test these ranges are
                           //!< going outside of original data object size which should raise exception while test.
    ...
    EXPECT_ANY_THROW(
        dObj2_test1 = this->dObj2.at(
            ranges1)); //!< Expect an exception to be raised because ranges are outside of original data object size

    //!< For 3 dimension Data Objects
    ito::DataObject dObj3_test1;
    ito::Range
        ranges_3d[3]; //!< Range variable to be used as parameter with at() function. In this test these ranges are
                      //!< going outside of original data object size which should raise exception while test.
    ...
    EXPECT_ANY_THROW(
        dObj3_test1 = this->dObj3.at(
            ranges_3d)); //!< Expect an exception to be raised because ranges are outside of original data object size
}

...
{
    int temp=0;
    unsigned int res1_str[] = {41,42,43,44,45,51,52,53,54,55};            //!< Expected element values for this->dObj2_dr after copying ROI of this->dObj2_sr by copyTo() function into it.
    unsigned int res2_str[] = {32,33,37,38,42,43,57,58,62,63,67,68};    //!< Expected element values for this->dObj3_dr after copying ROI of this->dObj3_sr by copyTo() function into it.
        for(int i =0;i<10;i++) 
        {
            for(int j=0;j<10;j++)
    ...
    int matLimits3d[] = {-1,-1,-1,-1,-2,-1};        //!< defining offsets for ROI of 3 dimensional data object this->dObj3_sr
    int matLimits5d[] = {-4,-4,-1,-4,-2,-3,-1,-1,-2,-1};        //!< defining offsets for ROI of 5 dimensional data object this->dObj4_sr
    ...
    EXPECT_EQ(0,this->dObj1_d.getDims());                    //!< Testing if dimension of this->dObj1_d is same as dimension of ROI of this->dObj1_s.
    EXPECT_EQ(this->dObj1_s.getSize(),this->dObj1_d.getSize() );    //!< Testing if size of this->dObj1_d is same as dimension of ROI of this->dObj1_s.
    ...
    EXPECT_EQ(this->dObj3_s.getSize(), this->dObj3_d.getSize() );    //!< Testing if the size of ROI of copied Data Object is same as the ROI of original Data Object.
    ...
    EXPECT_EQ(this->dObj4_s.getSize(), this->dObj4_d.getSize() );    //!< Testing if the size of ROI of copied Data Object is same as the ROI of original Data Object.
    ...
    //!< Testing functionality of copyTo() function for 2 dimensional Data Object with regionOnly parameter = True.
    this->dObj2_sr.copyTo(this->dObj2_dr,true);   //!< Copying only ROI from this->dObj2_sr to this->dObj2_dr using copyto(...) method with its regionOnly parameter as "true".
    EXPECT_EQ(this->dObj2_sr.getDims() ,this->dObj2_dr.getDims() );    //!< Testing if the dimensions of ROI of copied Data Object are same as the ROI of original Data Object.
    EXPECT_EQ(this->dObj2_sr.getSize(), this->dObj2_dr.getSize() );    //!< Testing if the size of ROI of copied Data Object is same as the ROI of original Data Object.
    for(int i =0;i<2;i++) 
        {
            for(int j=0;j<5;j++)EXPECT_EQ(this->dObj2_dr.template at<TypeParam>(i,j),cv::saturate_cast<TypeParam>(res1_str[i*5+j])); //!< Check if the values of elements in ROI of this->dObj2_dr are same as in the original this->dObj2_s.
        }

    //!< Testing functionality of copyTo() function for 3 dimensional Data Object with regionOnly parameter = True.
    this->dObj3_sr.copyTo(this->dObj3_dr,true);                        //!< copying this->dObj3_sr into this->dObj3_dr using copyTo(...) method with its regionOnly parameter as "true".
    EXPECT_EQ(this->dObj3_sr.getDims() ,this->dObj3_dr.getDims() );    //!< Testing if the dimensions of ROI of copied Data Object are same as the ROI of original Data Object.
    EXPECT_EQ(this->dObj3_sr.getSize() ,this->dObj3_dr.getSize() );     //!< Testing if the size of ROI of copied Data Object is same as the ROI of original Data Object.
    ...
    for(int i =0;i<2;i++) 
        {
            ...
            {  
                for(int k=0;k<2;k++)
                {
                    EXPECT_EQ(this->dObj3_dr.template at<TypeParam>(i,j,k),cv::saturate_cast<TypeParam>(res2_str[temp++]) );   //!< Checking if elements of ROI of original Data Object this->dObj3_sr has been changed while copying it to this->dObj3_dr.
                }
            }
        }

    //!< Testing functionality of copyTo() function for 5 dimensional Data Object with regionOnly parameter = True.
    this->dObj4_sr.copyTo(this->dObj4_dr,true);                        //!< copying this->dObj4_sr into this->dObj4_dr using copyTo(...) function with its regionOnly parameter as "true".
    EXPECT_EQ(this->dObj4_sr.getDims(),this->dObj4_dr.getDims() );    //!< Testing if the dimensions of ROI of copied Data Object are same as the ROI of original Data Object.
    EXPECT_EQ(this->dObj4_sr.getSize() ,this->dObj4_dr.getSize() );    //!< Testing if the size of ROI of copied Data Object is same as the ROI of original Data Object.
}
...
TYPED_TEST(copyTests, copyTo_False_Test)
{
    ...
    for(int i =0;i<2;i++) 
        {
            for(int j=0;j<5;j++)EXPECT_EQ(this->dObj2_dr.template at<TypeParam>(i,j),cv::saturate_cast<TypeParam>(res1_str[i*5+j])); //!< Check if the values of elements in ROI are same as in the original
        }
    ...
    this->dObj2_dr.adjustROI(2,matLimits2d_2);
        ...
        {
            for(int j=0;j<10;j++)
            {EXPECT_EQ( this->dObj2_s.template at<TypeParam>(i,j),this->dObj2_dr.template at<TypeParam>(i,j) );}   //!< checking if the original values of all elements retained after expanding ROI same as original Data Object this->dObj2_s.
        }
    ...
    this->dObj3_dr.adjustROI(3,matLimits3d_2);   //!< adjust ROI back to original size of source Data Object this->dObj3_s.
    ...
    for(int i =0;i<4;i++)        
        {
            ...
            {  
                for(int k=0;k<5;k++)
                {
                    EXPECT_EQ(this->dObj3_dr.template at<TypeParam>(i,j,k),this->dObj3_s.template at<TypeParam>(i,j,k) );   //!< Checking if elements of ROI of original Data Object this->dObj3_dr has been changed while adusting the size of ROI back to original.
                }
            }
        }
...
{                            
    ...
    for(int i=0; i<dim1; i++)
    {
        ...
        this->dObj4_dr.adjustROI(5,matLimits5d_2);    //!< adjusting ROI of dObj5 with general 2 parameter adjustROI method to desired position
    ...
    for(int i=0; i<dim1; i++)
    {
        for(int j=0; j<dim2;j++)
        {
            for(int k=0; k<dim3;k++)
            {
                ...
                for(int l=0; l<dim4;l++)
                {        
                    rowPtr1_dr= (TypeParam*)this->dObj4_dr.rowPtr(dataIdx,l);        //!< using row pointer accessing the data of data object this->dObj4_dr

                    for(int m=0; m<dim5;m++)
                    {
                        EXPECT_EQ(rowPtr1_dr[m],cv::saturate_cast<TypeParam>(this->calcUniqueValue5D(i,j,k,l,m)));    //!< checking if the ROI  of this->dObj4_dr retains same after applying copyTo(...) function.
                    }
                }
            }
        }

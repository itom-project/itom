...
TYPED_TEST(IsZeroValueRealTest, checkZeroValueReal_Test)
{
    ...
    {
        EXPECT_TRUE(ito::isZeroValue<TypeParam>(
            ZeroVal,
            0)); //!< isZeroValue<_Tp>(...) function must return true if the variable passed contains value '0'.
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            NonZeroVal1,
            0)); //!< isZeroValue<_Tp>(...) function must return false if the variable passed contains non-zero value.
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            NonZeroVal2,
            0)); //!< isZeroValue<_Tp>(...) function must return false if the variable passed contains non-zero value.
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
    ...
    else
    {
        ...
        EXPECT_TRUE(ito::isZeroValue<TypeParam>(
            ZeroVal, std::numeric_limits<TypeParam>::epsilon())); //!< isZeroValue<_Tp>(...) function must return true
                                                                  //!< if the variable passed contains value '0'.
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            min,
            std::numeric_limits<TypeParam>::epsilon())); //!< Note:------> Test is failing .. It should return false as
                                                         //!< the variable value is on the boundary "-epsilon".
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            max,
            std::numeric_limits<TypeParam>::epsilon())); //!< Note:------> Test is failing .. It should return false as
                                                         //!< the variable value is on the boundary "epsilon".
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            min + 1, std::numeric_limits<TypeParam>::epsilon())); //!< isZeroValue<_Tp>(...) function must return false
                                                                  //!< if the variable contains value lying outside of
                                                                  //!< boundary of 0 to epsilon.
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            max - 1, std::numeric_limits<TypeParam>::epsilon())); //!< isZeroValue<_Tp>(...) function must return false
                                                                  //!< if the variable contains value lying outside of
                                                                  //!< boundary of 0 to epsilon.
    }
}
...
TYPED_TEST(IsZeroValueComplexTest, checkZeroValueComplex_Test)
{
    ...
    if (!std::numeric_limits<TypeParam>::is_exact)
    {
        ...
        EXPECT_TRUE(ito::isZeroValue<TypeParam>(
            ZeroVal2,
            epsilon2)); /*!< Test of isZeroValue() function for real part of Complex Type Variable with zero value.  */
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(NonZeroVal4,
                                                 epsilon2)); /*!< Test of isZeroValue() function for real part of
                                                                Complex Type Variable with nonzero positive value. */
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(
            NonZeroVal5, epsilon2)); /*!< Test of isZeroValue() function for Complex Type variable with zero real part
                                        and nonzero negative imaginary part. */
        EXPECT_FALSE(ito::isZeroValue<TypeParam>(NonZeroVal6,
                                                 epsilon2)); /*!< Test of isZeroValue() function for real part of
                                                                Complex Type Variable with nonzero negative value. */
    }
}
...
TYPED_TEST(IsZeroValueRGBATest, checkZeroValueRGBA_Test)
{
    ...
    {
        EXPECT_TRUE(ito::isZeroValue<ito::Rgba32>(
            zeroVal1, epsilon1)); /*!< Test of isZeroValue() function for default constructor of rgba.  */
        EXPECT_TRUE(ito::isZeroValue<ito::Rgba32>(
            zeroVal2, epsilon1)); /*!< Test of isZeroValue() function for rgba constructor of rgba.  */
        EXPECT_TRUE(ito::isZeroValue<ito::Rgba32>(
            zeroVal3, epsilon1)); /*!< Test of isZeroValue() function for uint32 constructor of rgba.  */
        EXPECT_TRUE(ito::isZeroValue<ito::Rgba32>(
            zeroVal4, epsilon1)); /*!< Test of isZeroValue() function for int32 constructor of rgba.  */
        EXPECT_TRUE(ito::isZeroValue<ito::Rgba32>(
            zeroVal5, epsilon1)); /*!< Test of isZeroValue() function for int32 constructor of rgba.  */

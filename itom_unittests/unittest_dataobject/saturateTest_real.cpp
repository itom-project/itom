...
/*!
    This test checks if the saturation limits for for all real data types as defined in std library and openCv library
   match or not .
*/
TYPED_TEST(SaturateTestReal, checkSaturateBoundaries)
{
    ...
    else
    {
        ...
        EXPECT_FLOAT_EQ(cv::saturate_cast<ito::float32>((ito::float64)std::numeric_limits<ito::float64>::max()),
                        std::numeric_limits<ito::float32>::max());

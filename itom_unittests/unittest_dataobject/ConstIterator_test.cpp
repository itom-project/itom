...
TYPED_TEST(const_iterator_test, constIterator_test_2d)
{
    ...
    TypeParam objdata2d[273];    //!< This array holds the data of data object this->dObj2d with size dim1_2d x dim2_2d.
    TypeParam *dataptr2d; 
    ...
    temp=0;
    for(it_2d=this->dObj2d.constBegin();it_2d!=this->dObj2d.constEnd();++it_2d)
    {
        // std::cout << *((TypeParam*)(*it)) << std::constEndl;
        EXPECT_EQ(cv::saturate_cast<TypeParam>(*((TypeParam*)(*it_2d))),cv::saturate_cast<TypeParam>(objdata2d[temp++])); //!< Testing the functionality of declared DObjConstIterator by comparing the values of each element of data object this->dObj2d with the same values stored in array objdata2d using iterator it_2d .
    }     
}
...
TYPED_TEST(const_iterator_test, constIteratorROI_test_2d)
{
    ...
    TypeParam objdata2d[273];    //!< This array holds the data of data object this->dObj2d with size dim1_2d x dim2_2d. The size of this array is more than enough because the ROI of this->dObj2d will be shrunk with the use of adjustROI method during the test.
    TypeParam *dataptr2d; 
    dataptr2d = objdata2d; //!< Now pointer dataptr2d points to the array objdata2d.
    ito::DObjConstIterator it_2d;    //!< Declaration of DObjConstIterator
...
TYPED_TEST(const_iterator_test, constIterator_test_3d)
{
    ...
    TypeParam objdata3d[500];    //!< This array holds the data of data object this->dObj3d with size dim1_3d x dim2_3d x dim3_3d.
    TypeParam *dataptr3d;        
    ...
    temp=0;
    for(it_3d=this->dObj3d.constBegin();it_3d!=this->dObj3d.constEnd();++it_3d)
    {
        EXPECT_EQ(cv::saturate_cast<TypeParam>(*((TypeParam*)(*it_3d))),cv::saturate_cast<TypeParam>(objdata3d[temp++]));    //!< Testing the functionality of declared DObjConstIterator by comparing the values of each element of data object this->dObj3d with the same values stored in array objdata3d using iterator it_3d.
    }
}
...
TYPED_TEST(const_iterator_test, constIterator_test_4d)
{
            ...
            for(int j=0; j<dim2;j++)
            {
                ...
                {        
                    ...
                    for(int l=0; l<dim4;l++)
                    {
                        ...
                        *dataptr4d = cv::saturate_cast<TypeParam>(dim1*i*j*k+dim2*j*k+dim3*k+l);        //!< Defining the array with the same data values as in this->dObj4d for test purpose.
                        dataptr4d++;
                    }
                }
            }
    ...
    temp=0;
    for(it_4d=this->dObj4d.constBegin();it_4d!=this->dObj4d.constEnd();++it_4d)
    {
        EXPECT_EQ(cv::saturate_cast<TypeParam>(*((TypeParam*)(*it_4d))),cv::saturate_cast<TypeParam>(objdata4d[temp++]));    //!< Testing the functionality of declared DObjConstIterator by comparing the values of each element of data object this->dObj4d with the same values stored in array objdata4d using iterator it_4d.
    }
}

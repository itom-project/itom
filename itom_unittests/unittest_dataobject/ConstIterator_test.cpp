...
/*! \class const_iterator_test
    ...
    This test is a group of 6 tests for 2, 3 and 4 dimensional data objects. In each test, an iterator object of type
   DObjConstIterator is declared and data values of each element of data object is being fetched using this iterator.
   Later on test is performed such that data values fetched by iterators should match the original values given to each
   data object. The test is performed for data objects with no change in ROI as well as data objects with changed ROI.
*/
    ...
    for (it_2d = this->dObj2d.constBegin(); it_2d != this->dObj2d.constEnd(); ++it_2d)
    {
        ...
        EXPECT_EQ(cv::saturate_cast<TypeParam>(*((TypeParam *)(*it_2d))),
                  cv::saturate_cast<TypeParam>(
                      objdata2d[temp++])); //!< Testing the functionality of declared DObjConstIterator by comparing the
...
TYPED_TEST(const_iterator_test, constIteratorROI_test_2d)
{
    ...
    TypeParam objdata2d[273]; //!< This array holds the data of data object this->dObj2d with size dim1_2d x dim2_2d.
                              //!< The size of this array is more than enough because the ROI of this->dObj2d will be
                              //!< shrunk with the use of adjustROI method during the test.
    TypeParam *dataptr2d;
    dataptr2d = objdata2d;                //!< Now pointer dataptr2d points to the array objdata2d.
    ito::DObjConstIterator it_2d;         //!< Declaration of DObjConstIterator
...
TYPED_TEST(const_iterator_test, constIterator_test_3d)
{
    ...
    for (it_3d = this->dObj3d.constBegin(); it_3d != this->dObj3d.constEnd(); ++it_3d)
    {
        EXPECT_EQ(cv::saturate_cast<TypeParam>(*((TypeParam *)(*it_3d))),
                  cv::saturate_cast<TypeParam>(
                      objdata3d[temp++])); //!< Testing the functionality of declared DObjConstIterator by comparing the
...
TYPED_TEST(const_iterator_test, constIterator_test_4d)
{
            ...
            for (int k = 0; k < dim3; k++)
            {
                ...
                for (int l = 0; l < dim4; l++)
                {
                    ...
                    *dataptr4d = cv::saturate_cast<TypeParam>(
                        ...
                        l); //!< Defining the array with the same data values as in this->dObj4d for test purpose.
                    dataptr4d++;
                }
            }
        }
    ...
    for (it_4d = this->dObj4d.constBegin(); it_4d != this->dObj4d.constEnd(); ++it_4d)
    {
        EXPECT_EQ(cv::saturate_cast<TypeParam>(*((TypeParam *)(*it_4d))),
                  cv::saturate_cast<TypeParam>(
                      objdata4d[temp++])); //!< Testing the functionality of declared DObjConstIterator by comparing the

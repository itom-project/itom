...
class ItomJediLibTest(unittest.TestCase):
    ...
    def _assertOneHelpEntry(
        self, tooltips, description, docstrstart, descStartsWith=False
    ):
...
dataObject.ones([2, 2])"""
    ...
    def test_help_notypehints(self):
        ...
        h = jedilib.get_help(
            doc, 10, 6, path=p
        )  # var, no typehints, one expr: type: int
        ...
        if jedi.__version__ > "0.18.0":
            self._assertOneHelpEntry(
                h, "def shape", "shape: tuple\n\ntuple of int : Gets the shape", True
            )
        else:

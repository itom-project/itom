...
if [[ "$unamestr" == 'Darwin' ]]; then
	...
	echo "To be able to use python 3, which is required when using itom, we will set aliases for python and pip."
	...
	if echo "$answer2" | grep -iq "^y" ;then
		...
		echo " "
		echo "The default Python version on OS X is 2.x. Since |itom| is using Python 3.x you installed in the previous step but it isn't recommended to replace version 2.x with 3.x. We will set an alias for python3, so when entered python in a terminal session, python3 will be called."
		echo "To edit you aliases execute the following commands. The same thing must be done for pip and easy_install."
		echo "Be advised to check the installed version number of python and change it when necessary."
		echo " "

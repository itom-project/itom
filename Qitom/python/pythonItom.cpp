    ...
    if (PyArg_ParseTuple(pArgs, "s", &filename) == false)
    {
        ...
        else if (PyObject_HasAttrString(obj, "__file__"))
        {
            ...
            else
            {
                PyErr_SetString(
                    PyExc_ValueError,
                    "__file__ attribute of given argument could not be parsed as string.");
                return NULL;
            }
        }
        else
        {
            PyErr_SetString(
                PyExc_ValueError,
                "Argument is no filename string and no other object that has a __file__ "
                "attribute.");
            return NULL;
        }
    }
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    pyPlotImage_doc, "plot(data, className = \"\", properties = {}) -> Tuple[int, plotItem] \n\
\n\
...
PyObject* PythonItom::PyPlotImage(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    // at first try to strictly convert to a point cloud or polygon mesh (non strict conversion not
    // available for this) if this fails, try to non-strictly convert to data object, such that
    // numpy arrays are considered as well.
#if ITOM_POINTCLOUDLIBRARY > 0
    dataCont = QSharedPointer<ito::PCLPointCloud>(
        PythonQtConversion::PyObjGetPointCloudNewPtr(data, true, ok));
    if (!ok)
    {
        dataCont = QSharedPointer<ito::PCLPolygonMesh>(
            PythonQtConversion::PyObjGetPolygonMeshNewPtr(data, true, ok));
    }
#else
    ok = false;
#endif
    ...
    if (!ok)
    {
        ...
        return PyErr_Format(
            PyExc_RuntimeError,
            "1st parameter (data) cannot be converted to dataObject, pointCloud or polygonMesh "
            "(%s).",
            retval2.errorMessage());
#else
        return PyErr_Format(
            PyExc_RuntimeError,
            "1st parameter (data) cannot be converted to dataObject (%s).",
            retval2.errorMessage());
#endif
    }
    ...
    *figHandle = 0; // new figure will be requested
    ...
    QMetaObject::invokeMethod(
        ...
        Q_ARG(ito::UiDataContainer&, xDataCont),
    ...
    // this is important such that one can connect to signals of the plot (e.g. userInteractionDone)
    // since signals can only be handled if the last item in the baseItem-chain is of type itom.ui
    // or itom.figure
    args2 = PyTuple_New(0);
    kwds2 = PyDict_New();
...
PyDoc_STRVAR(
    pyPlot1d_doc,
    "plot1(data, xData = None, className = \"\", properties = {}) -> Tuple[int, plotItem] \n\
\n\
...
PyObject* PythonItom::PyPlot1d(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    if (!ok)
    {
        ...
        if (ok && xData)
        {
            ...
            {
                PyErr_SetString(
                    PyExc_RuntimeError, "2nd parameter (xData) cannot be converted to dataObject.");
                return NULL;
            }
        }
    }
...
#else
        PyErr_SetString(
            PyExc_RuntimeError, "1st parameter (data) cannot be converted to dataObject.");
#endif
        return NULL;
    }
    ...
    *figHandle = 0; // new figure will be requested
    ...
    if (name.compare("2d", Qt::CaseInsensitive) == 0 ||
        name.compare("2.5d", Qt::CaseInsensitive) == 0)
    {
        PyErr_Format(
            PyExc_RuntimeError,
            "invalid className parameter %s. Use the plot, plot2 or plot25 command instead to get "
            "a other dimensional representation",
            className);
        return NULL;
    }
    ...
    {
        name = "1d:" + name; // to be sure, that only plots from the 1d category are used (className
                             // must be compatible to 1d -> checked in FigureWidget::plot
    }
...
PyObject* PythonItom::PyPlot2d(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    ito::UiDataContainer dataCont;
    ito::UiDataContainer
        xDataCont; //= QSharedPointer<ito::DataObject>(); //this is a null pointer
                   // at first try to strictly convert to a point cloud or polygon mesh (non strict
                   // conversion not available for this) if this fails, try to non-strictly convert
                   // to data object, such that numpy arrays are considered as well.
#if ITOM_POINTCLOUDLIBRARY > 0
    dataCont = QSharedPointer<ito::PCLPointCloud>(
        PythonQtConversion::PyObjGetPointCloudNewPtr(data, true, ok));
    if (!ok)
    ...
    *figHandle = 0; // new figure will be requested
    ...
    QSharedPointer<unsigned int> objectID(new unsigned int);
    QString name(className);
    if (name.compare("1d", Qt::CaseInsensitive) == 0 ||
        name.compare("2.5d", Qt::CaseInsensitive) == 0)
    {
        PyErr_Format(
            PyExc_RuntimeError,
            "invalid className parameter %s. Use the plot, plot1 or plot25 command instead to get "
    ...
    {
        name = "2d:" + name; // to be sure, that only plots from the 2d category are used (className
                             // must be compatible to 2d -> checked in FigureWidget::plot
    }
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    pyPlot25d_doc, "plot25(data, className = \"\", properties = {}) -> Tuple[int, plotItem] \n\
\n\
Plots a dataObject, pointCloud or polygonMesh in a new figure window. \n\
\n\
...
PyObject* PythonItom::PyPlot25d(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    *figHandle = 0; // new figure will be requested
    ...
    QString name(className);
    if (name.compare("1d", Qt::CaseInsensitive) == 0 ||
        name.compare("2d", Qt::CaseInsensitive) == 0)
    {
        PyErr_Format(
            PyExc_RuntimeError,
            "invalid className parameter %s. Use the plot, plot1 or plot2 command instead to get a "
    ...
    {
        name = "2.5d:" +
            name; // to be sure, that only plots from the 2.5d category are used
                  // (className must be compatible to 2.5d -> checked in FigureWidget::plot
    }
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    pyLiveImage_doc, "liveImage(cam, className = \"\", properties = {}) -> Tuple[int, plotItem] \n\
\n\
...
PyObject* PyWidgetOrFilterHelp(bool getWidgetHelp, PyObject* pArgs, PyObject* pKwds)
{
    ...
    char** kwlist =
        getWidgetHelp ? const_cast<char**>(kwlistWidget) : const_cast<char**>(kwlistFilter);
    ...
    if (!PyArg_ParseTupleAndKeywords(
            pArgs, pKwds, "|sii", kwlist, &filterstring, &retDict, &userwithinfos))
    {
        return NULL;
    }
    ...
    else
    {
        ...
        for (int n = 0; n < keyList.size(); n++)
        {
            ...
            if (!filteredKey.compare(namefilter, Qt::CaseSensitive) && !listonly && !userwithinfos)
            {
                ...
                if (!retDict)
                {
                    std::cout << contextName.toUpper().toLatin1().data()
                              << "NAME:    " << filteredKey.toLatin1().data() << "\n";
                }
                else
                {
                ...
                else
                {
                    ...
                    ito::AddInAlgo::FilterDefExt* fFuncExt =
                        dynamic_cast<ito::AddInAlgo::FilterDefExt*>(fFunc);
                    ...
                    if (!retDict)
                    {
                        ...
                        {
                            std::cout << "OBSERVATION: \n* No observer can be passed to this "
                                         "filter.\n* It does not provide any status "
                    ...
                    else
                    {
                        ...
                        {
                            item = PythonQtConversion::QByteArrayToPyUnicodeSecure(
                                "No observer. No status information. No cancellation.");
                            PyDict_SetItemString(resulttemp, "observation", item);
                            Py_DECREF(item);
                        }
                    }
                ...
                if (filterParams)
                {
                    ...
                    if (filterParams->paramsMand.size())
                    {
                        if (!retDict)
                        {
                            ...
                            resultmand =
                                printOutParams(&(filterParams->paramsMand), false, true, -1);
                            Py_DECREF(resultmand);
                        }
                        else
                        {
                            resultmand =
                                printOutParams(&(filterParams->paramsMand), false, true, -1, false);
                    ...
                    if (filterParams->paramsOpt.size())
                    {
                        if (!retDict)
                        {
                            std::cout << "\nOptional parameters:\n";
                            resultopt =
                                ito::printOutParams(&(filterParams->paramsOpt), false, true, -1);
                            Py_DECREF(resultopt);
                        }
                        else
                        {
                            resultopt = ito::printOutParams(
                                &(filterParams->paramsOpt), false, true, -1, false);
                    ...
                    if (filterParams->paramsOut.size())
                    {
                        if (!retDict)
                        {
                            std::cout << "\nOutput parameters:\n";
                            resultopt =
                                ito::printOutParams(&(filterParams->paramsOut), false, true, -1);
                            Py_DECREF(resultopt);
                        }
                        else
                        {
                            resultopt = ito::printOutParams(
                                &(filterParams->paramsOut), false, true, -1, false);
        ...
        {
            std::cout << contextName.toLatin1().data()
                      << ", which contain the given string: " << namefilter.toLatin1().data()
                      << "\n";
        }
        else if (!retDict)
        {
            ...
            if (!userwithinfos && !retDict)
            {
                std::cout << "\n"
                          << contextName.append("name")
                                 .leftJustified(longest_name + 1, ' ', false)
                                 .toLatin1()
                                 .data()
                          << " \tInfo-String\n";
            }
        ...
        for (int n = 0; n < keyList.size(); n++)
        {
            ...
            if (filteredKey.contains(namefilter, Qt::CaseInsensitive))
            {
                ...
                if (!retDict)
                {
                    if (userwithinfos)
                        std::cout
                            << contextName.toUpper().toLatin1().data() << "NAME:    "
                            << filteredKey.leftJustified(longest_name, ' ', false).toLatin1().data()
                            << " ";
                    else
                        std::cout
                            << filteredKey.leftJustified(longest_name, ' ', false).toLatin1().data()
                ...
                if (userwithinfos)
                {
                    ...
                    if (filterParams)
                    {
                        if (filterParams->paramsMand.size())
                        {
                            ...
                            {
                                resultmand = printOutParams(
                                    &(filterParams->paramsMand), false, true, -1, false);
                                PyDict_SetItemString(
                                    resulttemp, "Mandatory Parameters", resultmand);
                                Py_DECREF(resultmand);
                            }
                        }
                        else if (!retDict)
                        ...
                        if (filterParams->paramsOpt.size())
                        {
                            if (!retDict)
                            {
                                std::cout << "\nOptional parameters:\n";
                                resultopt =
                                    printOutParams(&(filterParams->paramsOpt), false, true, -1);
                                Py_DECREF(resultopt);
                            }
                            else
                            {
                                resultopt = printOutParams(
                                    &(filterParams->paramsOpt), false, true, -1, false);
                                PyDict_SetItemString(resulttemp, "Optional Parameters", resultopt);
                                Py_DECREF(resultopt);
                            }
                        ...
                        if (filterParams->paramsOut.size())
                        {
                            if (!retDict)
                            {
                                std::cout << "\nOutput parameters:\n";
                                resultopt =
                                    printOutParams(&(filterParams->paramsOut), false, true, -1);
                                Py_DECREF(resultopt);
                            }
                            else
                            {
...
PyDoc_STRVAR(
    pyFilterHelp_doc,
    "filterHelp(filterName = \"\", dictionary = 0, furtherInfos = 0) -> Optional[dict] \n\
\n\
...
PyDoc_STRVAR(
    pyWidgetHelp_doc,
    "widgetHelp(widgetName = \"\", dictionary = 0, furtherInfos = 0) -> Optional[dict] \n\
\n\
...
PyObject* PythonItom::PyPlotLoaded(PyObject* /*pSelf*/, PyObject* pArgs)
{
    ...
    ito::DesignerWidgetOrganizer* dwo =
        qobject_cast<ito::DesignerWidgetOrganizer*>(AppManagement::getDesignerWidgetOrganizer());
    if (!dwo)
    {
...
PyDoc_STRVAR(
    pyPlotHelp_doc,
    "plotHelp(plotName = \"\", dictionary = False) -> Optional[Union[List[str], dict]] \n\
\n\
...
PyObject* PythonItom::PyPlotHelp(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            pArgs, pKwds, "|sp", const_cast<char**>(kwlist), &plotName, &retDict))
    {
        return NULL;
    }
    ...
    if (!dwo)
    {
        ...
        plotName == NULL || strlen(plotName) == 0 || (strlen(plotName) == 1 && plotName[0] == '*'))
    {
        bool found = false;
        QList<ito::FigurePlugin> plugins = dwo->getPossibleFigureClasses(0, 0, 0);
    ...
    else
    {
        ...
        if (fig.factory)
        {
            ...
            ito::AbstractItomDesignerPlugin* fac =
                (ito::AbstractItomDesignerPlugin*)(fig.factory->instance());

            int version = fac->getVersion();
            QString versionStr = QString("%1.%2.%3")
                                     .arg(MAJORVERSION(version))
                                     .arg(MINORVERSION(version))
                                     .arg(PATCHVERSION(version));
            ...
            if (retDict)
            {
                item = PythonQtConversion::QStringToPyObject(fac->getDetailDescription()); // new
                                                                                           // ref
            ...
            QMetaObject::invokeMethod(
                uiOrg,
                "getObjectInfo",
                Q_ARG(const QString&, fig.classname),
                Q_ARG(bool, true),
                Q_ARG(ito::UiOrganizer::ClassInfoContainerList*, &objInfo),
                Q_ARG(ItomSharedSemaphore*, locker.getSemaphore()));
            ...
            for (mapIter = objInfo.begin(); mapIter != objInfo.end(); ++mapIter)
            {
                if (mapIter->m_type == ito::ClassInfoContainer::TypeClassInfo)
                {
                    if (retDict)
                    {
                        item = PythonQtConversion::QStringToPyObject(
                            mapIter->m_description); // new ref
                    ...
                    else
                    {
                        std::cout << mapIter->m_name.toLatin1().data() << " : "
                                  << mapIter->m_shortDescription.toLatin1().data() << "\n";
                    }
                }
            }
...
PyObject* PythonItom::PyPluginHelp(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            pArgs, pKwds, "s|p", const_cast<char**>(kwlist), &pluginName, &retDict))
    {
        return NULL;
    }
    ...
    case ito::typeActuator:
        ...
        {
            std::cout << "\n";
            std::cout << "\nFor more information use the member functions 'getParamListInfo()' and "
                         "'getExecFuncInfo()'\n\n";
        }
        break;
    ...
    case ito::typeAlgo: {
        if (pluginNum >= 0 && pluginNum < AIM->getAlgList()->size())
        {
            ...
            ito::AddInAlgo* algoInst =
                (ito::AddInAlgo*)((ito::AddInInterfaceBase*)AIM->getAddInPtr(pluginNum))
                    ->getInstList()[0];
            ...
            if (funcList.size() > 0)
            {
                ...
                if (retDict)
                {
                    ...
                    {
                        item = PythonQtConversion::QByteArrayToPyUnicodeSecure(
                            keyList.value(algos).toLatin1());
                        PyDict_SetItemString(
                            algorithmlist, keyList.value(algos).toLatin1().data(), item);
                        Py_DECREF(item);
                    }
            ...
            if (widgetList.size() > 0)
            {
                ...
                if (retDict)
                {
                    ...
                    {
                        item = PythonQtConversion::QByteArrayToPyUnicodeSecure(
                            keyList.value(widgets).toLatin1());
                        PyDict_SetItemString(
                            widgetlist, keyList.value(widgets).toLatin1().data(), item);
                        Py_DECREF(item);
                    }
                ...
                else
                {
                    ...
                    {
                        std::cout << "> " << widgets << "  "
                                  << keyList.value(widgets).toLatin1().data() << "\n";
                    }
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    pyItomVersion_doc, "version(dictionary = False, addPluginInfo = False) -> Optional[dict] \n\
\n\
...
PyObject* PythonItom::PyItomVersion(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
        ...
        if (aim != nullptr)
        {
            ...
            for (int i = 0; i < aim->getTotalNumAddIns(); i++)
            {
                ...
                if (curAddInInterface)
                {
                    ...
                    license =
                        PythonQtConversion::QStringToPyObject(curAddInInterface->getLicenseInfo());
    ...
    else
    {
        ...
        for (Py_ssize_t i = 0; i < size; i++)
        {
            ...
            maxLengthCol1 =
                std::min(longestKeySize + 2, maxLengthCol1); // +2 due to colon and space
            ...
            for (Py_ssize_t m = 0; m < PyList_Size(mySubKeys); ++m)
            {
                ...
                if (PyDict_Check(currentSubVal))
                {
                    ...
                    {
                        subVal =
                            QString("%1").arg(PythonQtConversion::PyObjGetInt(curVal, true, check));
                    }
                    else
                    {
                    ...
                    if (PyLong_Check(curVal))
                    {
                        subVal.append(QString("%1").arg(
                            PythonQtConversion::PyObjGetInt(curVal, true, check)));
                    }
                    else
                    {
                ...
                {
                    subVal = QString("%1").arg(
                        PythonQtConversion::PyObjGetInt(currentSubVal, true, check));
                }
                else
                {
                ...
                if (subVal.size() > maxLengthCol2)
                {
                    ...
                    if (lines.size() > 0)
                    {
                        ...
                        {
                            std::cout << empty.constData() << lines[lineIdx].toLatin1().data()
                                      << "\n";
                        }
                    }
                }
...
PyDoc_STRVAR(
    pyAddButton_doc,
    "addButton(toolbarName, buttonName, code, icon = \"\", argtuple = []) -> int \n\
\n\
...
PyObject* PythonItom::PyAddButton(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* kwds)
{
    ...
    {
        retValue +=
            RetVal(retError, 0, QObject::tr("Button must have a valid name.").toLatin1().data());
    }
    else
    {
    ...
    // afterwards (it corresponds to the pointer address of the corresponding QAction, casted to
    // size_t)
    ...
    if (!retValue.containsError())
    {
        ...
        if (mainWindow)
        {
            ...
            if (!locker->wait(2000))
            {
                unhashButtonOrMenuCode(
                    funcWeakRef); // if the function is already hashed, release it.
                PyErr_SetString(
                    PyExc_RuntimeError, "Timeout while waiting for button being added.");
                return NULL;
            }
            else
            {
        ...
        {
            PyErr_SetString(
                PyExc_RuntimeError, "Main window not available. Button cannot be added.");
            return NULL;
        }
    }
...
PyDoc_STRVAR(
    pyAddMenu_doc,
    "addMenu(type, key, name = \"\", code = \"\", icon = \"\", argtuple = []) -> int \n\
\n\
...
PyObject* PythonItom::PyAddMenu(PyObject* /*pSelf*/, PyObject* args, PyObject* kwds)
{
    ...
    {
        retValue += RetVal(
            retError, 0, QObject::tr("Menu element must have a valid key.").toLatin1().data());
    }
    else
    {
        // check type
            ...
            if (!code)
            {
                retValue += RetVal(
                    ...
                    QObject::tr("For menu elements of type 'BUTTON' any type of code "
                                "(String or callable method or function) must be indicated.")
                        .toLatin1()
                        .data());
            }
            else
        ...
        case 1: // SEPARATOR
        {
            ...
            if (ok && qcode != "")
            {
                retValue += RetVal(
                    retWarning,
                    0,
                    QObject::tr("A menu element of type 'separator' can not execute some code. "
            ...
            else if (!ok && code != NULL && code != Py_None)
            {
                retValue += RetVal(
                    retWarning,
                    0,
                    QObject::tr("A menu element of type 'separator' can not execute any function "
                                "or method. Code argument is ignored.")
                        .toLatin1()
                        .data());
            }
            break;
        ...
        case 2: // MENU
        {
            ...
            if (ok && qcode != "")
            {
                retValue += RetVal(
                    retWarning,
                    0,
                    QObject::tr("A menu element of type 'menu' can not execute some code. Code "
                                "argument is ignored.")
                        .toLatin1()
                        .data());
                qcode = "";
            ...
            else if (!ok && code != NULL && code != Py_None)
            {
                retValue += RetVal(
                    retWarning,
                    0,
                    QObject::tr("A menu element of type 'menu' can not execute any function or "
                                "method. Code argument is ignored.")
                        .toLatin1()
                        .data());
            }
            break;
    ...
    if (!retValue.containsError())
    {
        ...
        {
            PyErr_SetString(
                PyExc_RuntimeError, "Main window not available. Menu could not be added.");
            return NULL;
        }
    }
...
PyObject* PythonItom::PyRemoveMenu(PyObject* /*pSelf*/, PyObject* args, PyObject* kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "s", const_cast<char**>(kwlist), &keyName))
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwds, "I", const_cast<char**>(kwlist2), &menuHandle))
        {
            PyErr_SetString(
                PyExc_TypeError,
                "Wrong length or type of arguments. Type help(removeMenu) for more information.");
            return NULL;
        }
    }
    else
    ...
    {
        PyErr_SetString(
            PyExc_RuntimeError, "Main window not available. Menu could not be removed.");
        return NULL;
    }
...
/*static*/ PyObject* PythonItom::PyDumpMenusAndButtons(PyObject* pSelf)
{
    ...
    if (mainWindow)
    {
        ...
        else
        {
            ...
            PyObject* result =
                PyRun_String(totalString.toLatin1().data(), Py_single_input, globals, NULL);
            Py_XDECREF(result);
    ...
    {
        PyErr_SetString(
            PyExc_RuntimeError, "Main window not available. Dump could not be generated.");
        return NULL;
    }
}
...
/*static */ PyObject* PythonItom::PyGCStartTracking(PyObject* /*pSelf*/)
{
    ...
    if (gc)
    {
        ...
        for (Py_ssize_t i = 0; i < PyList_Size(obj_list); i++)
        {
            ...
            m_gcTrackerList[(size_t)t] =
                QString("%1 [%2]")
                    .arg(t->ob_type->tp_name)
                    .arg(PythonQtConversion::PyObjGetString(t, false, ok)); // t->ob_type->tp_name;
        }
...
/*static */ PyObject* PythonItom::PyGCEndTracking(PyObject* /*pSelf*/)
{
    ...
    {
        PyErr_SetString(
            PyExc_RuntimeError, "Tracker has not been started. Call gcStartTracking() first.");
        return NULL;
    }
    ...
    if (gc)
    {
        ...
        for (Py_ssize_t i = 0; i < PyList_Size(obj_list); i++)
        {
            ...
            if (n == 0)
            {
                temp = QString("%1 [%2]")
                           .arg(t->ob_type->tp_name)
                           .arg(PythonQtConversion::PyObjGetString(t, false, ok));
                std::cout << "New Element. Addr:" << (size_t)t
                          << " Type: " << temp.toLatin1().data() << "\n"
                          << std::endl;
            }
        }
...
PyDoc_STRVAR(
    autoReloader_doc,
    "autoReloader(enabled, checkFileExec = True, checkCmdExec = True, checkFctExec = False) \n\
\n\
...
PyObject* PythonItom::PySaveMatlabMat(PyObject* /*pSelf*/, PyObject* pArgs)
{
    ...
    //                   dict or sequence, whose elements will be put into a dict with
    //                   default-name-sets or
    ...
    if (!PyDict_Check(element))
    {
        if (!PyTuple_Check(element) && !PyList_Check(element))
        {
            ...
            else
            {
                ...
                if (!PyArg_ParseTuple(matrixNamesTuple, "s", &tempName))
                {
                    ...
                    PyErr_SetString(
                        PyExc_TypeError,
                        "if matrix name is indicated, it must be one unicode string");
                    return NULL;
                }
                Py_XDECREF(matrixNamesTuple);
            }
        ...
        else
        {
            ...
            else if (!PyArg_ParseTuple(matrixNames, "s", &tempName))
            {
                ...
                if (!PySequence_Check(matrixNames) ||
                    PySequence_Size(matrixNames) < PySequence_Size(element))
                {
                    Py_XDECREF(scipyIoModule);
                    PyErr_SetString(
                        PyExc_TypeError,
                        "if matrix name is indicated, it must be a sequence of unicode strings "
                        "(same length than elements)");
                    return NULL;
                }
                else
                {
                    for (Py_ssize_t i = 0; i < PySequence_Size(matrixNames); i++)
                    {
                        ...
                        if (!PyUnicode_Check(matrixNamesItem) && !PyBytes_Check(matrixNamesItem))
                        {
                            ...
                            PyErr_SetString(
                                PyExc_TypeError,
                                "each element of matrix names sequence must be a unicode object");
                            return NULL;
                        }
                        Py_XDECREF(matrixNamesItem);
                    }
...
PyObject* PythonItom::PyMatlabMatDataObjectConverter(PyObject* element)
{
    ...
    {
        newElement = PythonDataObject::PyDataObject_getTagDict(
            (PythonDataObject::PyDataObject*)element, NULL);
        // Py_INCREF(element);
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    pyLoadDataObject_doc, "loadDataObject(filename, dataObject, doNotAppendIDO = False) \n\
\n\
...
");
PyObject* PythonItom::PySetCentralWidgetsSizes(
    PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    {
        QMetaObject::invokeMethod(
            mainWindow, "setCentralWidgetsSizes", Q_ARG(QVector<int>, sizes_));
    }
    else
    {
        PyErr_SetString(PyExc_RuntimeError, "Main window is not available");
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    scaleValueAndUnit_doc,
    "scaleValueAndUnit(scaleableUnits, value, valueUnit) -> Tuple[float, str] \n\
\n\
...
PyObject* PythonItom::scaleValueAndUnit(PyObject* /*pSelf*/, PyObject* pArgs, PyObject* pKwds)
{
    ...
    return Py_BuildValue(
        "dN",
        valueOut,
        PythonQtConversion::QStringToPyObject(
            unitOut)); //"N" -> Py_BuildValue steals reference from QStringToPyObject
}
...
{
    return PythonQtConversion::QStringToPyObject(
...
PyObject* PythonItom::getQtToolPath(PyObject* /*pSelf*/, PyObject* pArgs)
{
    ...
    {
        PyErr_SetString(
            PyExc_FileExistsError, "Absolute path to desired binary could not be found.");
        return NULL;
    }
}
...
/*static*/ PyObject* PythonItom::compressData(PyObject* pSelf, PyObject* pArgs)
{
    ...
    if (level < -1 || level > 9)
    {
        PyErr_SetString(
            PyExc_RuntimeError,
            "Compression level must be -1 (default: level 6) or between 0 and 9");
        return NULL;
    }
    ...
    PyObject* pSelf, PyObject* pArgs, PyObject* pKwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            pArgs, pKwds, "s|s", const_cast<char**>(kwlist), &rccFileName, &mapRoot))
    {
        return NULL;
    }
...
//-------------------------------------------------------------------------------------
PyDoc_STRVAR(
    pyUnregisterResources_doc, "unregisterResource(rccFileName, mapRoot = \"\") -> bool \n\
\n\
...
PyObject* PythonItom::setApplicationCursor(PyObject* pSelf, PyObject* pArgs)
{
    ...
    {
        return PyErr_Format(
            PyExc_RuntimeError, "Cursor number must be in range [-1,%i]", Qt::LastCursor);
    }
...
PyObject* PythonItom::PySaveIDC(PyObject* pSelf, PyObject* pArgs, PyObject* pKwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            ...
            &PyDict_Type,
            &dict,
            &overwriteIfExists)) // all borrowed
    {
        return NULL;
    }
    ...
    if (pyEngine)
    {
        ...
        else
        {
            return PyErr_Format(
                PyExc_RuntimeError,
                "The file '%s' cannot be overwritten",
                info.absoluteFilePath().toUtf8().data());
        }
    }
    else
...
PyObject* PythonItom::userGetUserInfo(PyObject* /*pSelf*/)
{
    ...
    PyObject* item = PyUnicode_DecodeLatin1(
        userOrg->getCurrentUserName().toLatin1().data(),
        userOrg->getCurrentUserName().length(),
        NULL);
    ...
    // ID
    item = PyUnicode_DecodeLatin1(
        userOrg->getCurrentUserId().toLatin1().data(), userOrg->getCurrentUserId().length(), NULL);
...
int PyRgbaOptional_Converter(PyObject* object, PythonRgba::PyRgba** address)
{
    ...
    else
    {
        PyErr_SetString(
            PyExc_TypeError,
            "The arguments inverseColor1, inverseColor2 and invalidColor must be of type rgba or "
            "None.");
        *address = NULL;
        return 0;
    }
...
PyDoc_STRVAR(
    setPalette_doc,
    "setPalette(name, colorStops, inverseColor1 = None, inverseColor2 = None, invalidColor = None) \n\
\n\
...
{
    const char* kwlist[] = {
        "name", "colorStops", "inverseColor1", "inverseColor2", "invalidColor", NULL};
    char* name = NULL;
    ...
    for (int i = 0; i < length; ++i)
    {
        ...
        if (PyTuple_Check(subtuple))
        {
            ...
            if (!PyArg_ParseTuple(subtuple, "dO!", &pos, &PythonRgba::PyRgbaType, &color))
            {
                return PyErr_Format(
                    PyExc_RuntimeError,
                    "The %i. item of colorStops must be a tuple with a real value, followed by a "
                    "itom.rgba color.",
                    i);
            }
            ...
            if (i == 0)
            {
                ...
                {
                    return PyErr_Format(
                        PyExc_RuntimeError, "The position of the first color stop must be 0.0.");
                }
                else
                {
            ...
            else if (i == length - 1)
            {
                ...
                {
                    return PyErr_Format(
                        PyExc_RuntimeError, "The position of the last color stop must be 1.0.");
                }
                else
                {
                    stops.append(QPair<qreal, QColor>(pos, QColor(rgba.r, rgba.g, rgba.b, rgba.a)));
            ...
            else
            {
                if (pos < 0.0 || pos > 1.0)
                {
                    return PyErr_Format(
                        PyExc_RuntimeError,
                        "The position of any color stop must be in the range [0.0, 1.0].");
                }
                else
                {
                    found = false;
                    ...
                    for (int j = 1; j < stops.size(); ++j)
                    {
                        if (stops[j].first > pos)
                        {
                            stops.insert(
                                j - 1,
                                QPair<qreal, QColor>(pos, QColor(rgba.r, rgba.g, rgba.b, rgba.a)));
                            found = true;
                            break;
                        }
                    }
        ...
        else
        {
            return PyErr_Format(
                PyExc_RuntimeError,
                "The %i. item of colorStops must be a tuple with two values.",
                i);
        }
    }
    ...
    {
        return PyErr_Format(
            PyExc_RuntimeError, "colorStops must consist of at least two color stops.");
    }
...
PyObject* PythonItom::PyGetPalette(PyObject* pSelf, PyObject* pArgs)
{
    ...
    for (int elem = 0; elem < sharedPalette->getNumColorStops(); elem++)
    {
        ...
        ito::PythonRgba::PyRgba* rgb = (ito::PythonRgba::PyRgba*)ito::PythonRgba::PyRgba_new(
        ...
        rgb->rgba.b = c.blue();
        PyTuple_SetItem(
            subtuple, 0, PyFloat_FromDouble(sharedPalette->getPos(elem))); // steals a reference
    ...
    PyObject* name_ =
        PythonQtConversion::QByteArrayToPyUnicodeSecure(sharedPalette->getName().toLatin1());
    ...
    ito::PythonRgba::PyRgba* inverseColor1 = (ito::PythonRgba::PyRgba*)ito::PythonRgba::PyRgba_new(
        &ito::PythonRgba::PyRgbaType, NULL, NULL); // new ref
    ...
    ito::PythonRgba::PyRgba* inverseColor2 = (ito::PythonRgba::PyRgba*)ito::PythonRgba::PyRgba_new(
        &ito::PythonRgba::PyRgbaType, NULL, NULL); // new ref
    ...
    ito::PythonRgba::PyRgba* invalidColor = (ito::PythonRgba::PyRgba*)ito::PythonRgba::PyRgba_new(
        &ito::PythonRgba::PyRgbaType, NULL, NULL); // new ref
...
PyMethodDef PythonItom::PythonMethodItom[] = {
    ...
    {"scriptEditor",
     (PyCFunction)PythonItom::PyOpenEmptyScriptEditor,
     METH_NOARGS,
    ...
    {"liveImage",
     (PyCFunction)PythonItom::PyLiveImage,
     METH_VARARGS | METH_KEYWORDS,
     pyLiveImage_doc},
    {"close",
     (PyCFunction)PythonFigure::PyFigure_close,
     METH_VARARGS,
     pyItom_FigureClose_doc}, /*class static figure.close(...)*/
    {"filter", (PyCFunction)PythonItom::PyFilter, METH_VARARGS | METH_KEYWORDS, pyFilter_doc},
    {"filterHelp",
     (PyCFunction)PythonItom::PyFilterHelp,
     METH_VARARGS | METH_KEYWORDS,
     pyFilterHelp_doc},
    {"widgetHelp",
     (PyCFunction)PythonItom::PyWidgetHelp,
     METH_VARARGS | METH_KEYWORDS,
     pyWidgetHelp_doc},
    {"pluginHelp",
     (PyCFunction)PythonItom::PyPluginHelp,
     METH_VARARGS | METH_KEYWORDS,
     pyPluginHelp_doc},
    {"aboutInfo",
     (PyCFunction)PythonItom::PyAboutInfo,
     METH_VARARGS | METH_KEYWORDS,
     pyAboutInfo_doc},
    ...
    {"version",
     (PyCFunction)PythonItom::PyItomVersion,
     METH_VARARGS | METH_KEYWORDS,
     pyItomVersion_doc},
    {"saveDataObject",
     (PyCFunction)PythonItom::PySaveDataObject,
     METH_VARARGS | METH_KEYWORDS,
     pySaveDataObject_doc},
    {"loadDataObject",
     (PyCFunction)PythonItom::PyLoadDataObject,
     METH_VARARGS | METH_KEYWORDS,
     pyLoadDataObject_doc},
    {"setCentralWidgetsSizes",
     (PyCFunction)PythonItom::PySetCentralWidgetsSizes,
     METH_VARARGS | METH_KEYWORDS,
     pySetCentralWidgetsSizes_doc},
    {"addButton",
     (PyCFunction)PythonItom::PyAddButton,
     METH_VARARGS | METH_KEYWORDS,
     pyAddButton_doc},
    ...
    {"removeMenu",
     (PyCFunction)PythonItom::PyRemoveMenu,
     METH_VARARGS | METH_KEYWORDS,
     pyRemoveMenu_doc},
    {"dumpButtonsAndMenus",
     (PyCFunction)PythonItom::PyDumpMenusAndButtons,
     METH_NOARGS,
     pyDumpButtonsAndMenus_doc},
    ...
    {"scaleValueAndUnit",
     (PyCFunction)PythonItom::scaleValueAndUnit,
     METH_VARARGS | METH_KEYWORDS,
     scaleValueAndUnit_doc},
    {"getDefaultScaleableUnits",
     (PyCFunction)PythonItom::getDefaultScaleableUnits,
     METH_NOARGS,
     getDefaultScaleableUnits_doc},
    ...
    {"gcStartTracking",
     (PyCFunction)PythonItom::PyGCStartTracking,
     METH_NOARGS,
     pyGCStartTracking_doc},
    ...
    {"setApplicationCursor",
     (PyCFunction)PythonItom::setApplicationCursor,
     METH_VARARGS,
     setApplicationCursor_doc},
    ...
    {"userIsDeveloper",
     (PyCFunction)PythonItom::userCheckIsDeveloper,
     METH_NOARGS,
     pyCheckIsDeveloper_doc},
    ...
    {"autoReloader",
     (PyCFunction)PythonItom::PyAutoReloader,
     METH_VARARGS | METH_KEYWORDS,
     autoReloader_doc},
    ...
    {"setPalette",
     (PyCFunction)PythonItom::PySetPalette,
     METH_VARARGS | METH_KEYWORDS,
     setPalette_doc},
    ...
    {"registerResource",
     ...
     METH_VARARGS | METH_KEYWORDS,
     pyRegisterResources_doc},
    {"unregisterResource",
     (PyCFunction)PythonItom::PyUnregisterResource,
     METH_VARARGS | METH_KEYWORDS,
     pyUnregisterResources_doc},
    {NULL, NULL, 0, NULL}};

PyModuleDef PythonItom::PythonModuleItom = {
    PyModuleDef_HEAD_INIT, "itom", NULL, -1, PythonItom::PythonMethodItom, NULL, NULL, NULL, NULL};
...
/*static*/ ito::FuncWeakRef* PythonItom::hashButtonOrMenuCode(
    PyObject* code, PyObject* argtuple, ito::RetVal& retval, QString& codeString)
{
    ...
    if (pyEngine)
    {
        ...
        {
            retval +=
                ito::RetVal(ito::retError, 0, QObject::tr("No code is given").toLatin1().data());
        }
        else if (PyMethod_Check(code) || PyCFunction_Check(code) || PyCallable_Check(code))
        {
            PyObject* arguments = PyTuple_New(1);
            Py_INCREF(code);
            PyTuple_SetItem(arguments, 0, code); // steals ref
            ito::PythonProxy::PyProxy* proxy = (ito::PythonProxy::PyProxy*)PyObject_CallObject(
                (PyObject*)&PythonProxy::PyProxyType, arguments); // new ref
            Py_DECREF(arguments);
            ...
            if (proxy)
            {
                if (argtuple)
                {
                    ...
                    else
                    {
                        retval += RetVal(
                            retError,
                            0,
                            QObject::tr("Given argument must be of type tuple or list.")
                                .toLatin1()
                                .data());
                        Py_DECREF(proxy);
                    }
                ...
                if (!retval.containsError())
                {
                    ...
                    ref = &(pyEngine->m_pyFuncWeakRefHashes.insert(
                                funcID, ito::FuncWeakRef(proxy, argtuple)))
                               .value();
            ...
            else
            {
                retval += ito::RetVal(
                    ito::retError,
                    0,
                    QObject::tr("Proxy object of function or method could not be created.")
                        .toLatin1()
                        .data());
            }
        }
        else
        {
            ...
            if (!ok)
            {
                retval += ito::RetVal(
                    ito::retError,
                    0,
                    QObject::tr("Code is no function or method call and no executable code string")
                        .toLatin1()
                        .data());
            }
        }
    ...
    {
        retval += ito::RetVal(
            ito::retError, 0, QObject::tr("Python engine is not available").toLatin1().data());
    }

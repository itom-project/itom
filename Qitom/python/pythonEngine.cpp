...
//            PyEval_InitThreads();                                                   //!< prepare Python multithreading
			...
			if  ((tretVal = runString("import sys")) != ito::retOk)
				(*retValue) += ito::RetVal(ito::retError, 0, "error importing sys in start python engine\n");
            if ((tretVal = runString("import itom")) != ito::retOk)
				(*retValue) += ito::RetVal(ito::retError, 0, "error importing itom in start python engine\n");
			if ((tretVal = runString("sys.stdout = itom.pythonStream(1)")) != ito::retOk)
				(*retValue) += ito::RetVal(ito::retError, 0, "error redirecting stdout in start python engine\n");
            if ((tretVal = runString("sys.stderr = itom.pythonStream(2)")) != ito::retOk)
				(*retValue) += ito::RetVal(ito::retError, 0, "error redirecting stderr in start python engine\n");
...
*/
            ...
            {
				(*retValue) += ito::RetVal(ito::retError, 0, "the module itoFunctions could not be loaded.");
            ...
            {
				(*retValue) += ito::RetVal(ito::retError, 0, "the module itoDebugger could not be loaded.");
...
ito::RetVal PythonEngine::stringEncodingChanged()
{
    ...
    else
    {
        ...
        foreach(const QByteArray &ba, qtCodecNames)
        {
        ...
        if(!found)
        {
            retval += ito::RetVal::format(ito::retWarning,0, "Qt text encoding %s not compatible to python. Python encoding is set to UTF-8", qtCodecNames.last().data() );
            encodingType = PythonQtConversion::utf_8;
            encodingName = "utf_8";
        }
    }
...
//----------------------------------------------------------------------------------------------------------------------------------
RetVal PythonEngine::runString(const char *command)
{
    RetVal retValue = RetVal(retOk);
    ...
    {
        QString tempCmd(command);
        if (tempCmd.contains('\n')) //multi-line commands must have the Py_file_input flag
        {
            result = PyRun_String(command, Py_file_input /*Py_single_input*/ , mainDict, localDict); //Py_file_input is used such that multi-line commands (separated by \n) are evaluated
        }
        ...
        {
            result = PyRun_String(command, Py_single_input, mainDict , localDict); //Py_file_input is used such that multi-line commands (separated by \n) are evaluated
...
RetVal PythonEngine::runPyFile(char* pythonFileName)
{
    ...
    else if (method == 2)
    {
        ...
        {
            result = PyObject_CallMethod(itomDbgInstance, "runScript", "s", pythonFileName);
...
//----------------------------------------------------------------------------------------------------------------------------------
RetVal PythonEngine::debugFile(char* pythonFileName)
{
    PyObject* result = NULL;
    RetVal retValue = RetVal(retOk);
    ...
    else
    {
        ...
        result = PyObject_CallMethod(itomDbgInstance, "debugScript", "s", pythonFileName);
...
//----------------------------------------------------------------------------------------------------------------------------------
RetVal PythonEngine::debugString(const char *command)
{
    PyObject* result = NULL;
    RetVal retValue = RetVal(retOk);
    ...
    else
    {
        ...
        result = PyObject_CallMethod(itomDbgInstance, "debugString", "s", command);
...
void PythonEngine::pythonDebugFunction(PyObject *callable, PyObject *argTuple)
{
    ...
    case pyStateDebuggingWaitingButBusy:
        ...
        std::cout << "it is not possible to debug a function call while another python function is being debugged or run. Function will be executed directly" << std::endl;
    ...
    else
    {
        ...
        else
        {
            for(int i=0; i<names.size();i++)
            {
                ...
                if(value == NULL)
                {
                    retVal += ito::RetVal::format(ito::retError,0,tr("item '%s' does not exist in workspace.").toAscii().data(), names[i].toAscii().data());
                    break;
                }
                else
                {

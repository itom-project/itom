...
public:
    ...
    RetVal saveAllScripts(
        bool askFirst = true, bool ignoreNewScripts = false, int* saveScriptState = NULL);
    ...
    inline const ScriptEditorActions& getScriptEditorActions() const
    {
        return m_commonScriptEditorActions;
    }
    ...
    ScriptDockWidget* activateOpenedScriptByFilename(
...
private:
    ...
    QList<ScriptDockWidget*> m_scriptDockElements; //! list with references to all ScriptDockWidgets
                                                   //! (docked or windows-style)
    ...
    mutable QMutex m_scriptStackMutex; //! mutex locking any changes to m_scriptDockElements. This
                                       //! mutex can also be changed in const methods
    ...
    QList<GoBackNavigationItem>
        m_goBackNavigationHistory; //! history of go back navigation items. Newer items are at the
                                   //! end of the list. The list is limited to a number of maximum
                                   //! items.
...
signals:
    void addScriptDockWidgetToMainWindow(
        AbstractDockWidget* dockWidget,
        Qt::DockWidgetArea
            area); //! signal emitted if dockWidget should be added to docking area in main window
    void removeScriptDockWidgetFromMainWindow(
        AbstractDockWidget*
            dockWidget); //! signal emitted if dockWidget should be removed from main window

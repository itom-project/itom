...
void DialogLoadedPlugins::init()
{
    ...
    foreach (const PluginLoadStatus& item, m_content)
    {
        ...
        QChar sortElement = ' '; // This character is only in a column if there is an icon... this
                                 // makes it easy to sort with the standard function
        ...
        for (int i = 0; i < item.messages.size(); i++)
        {
            ...
            else if (message->first & ito::plsfWarning)
            {
                ...
                {
                    m_items.append(
                        QPair<int, QTreeWidgetItem*>(ito::plsfWarning | ito::plsfRelDbg, child));
                }
                else
                {
...
{
    int stateCount[4] = {
        0, 0, 0, 0}; // we need 1: plsfOK, 2: plsfWarning, 4: plsfError, 8: plsfIgnored
    ...
    QRegularExpression rx(
        ".*" + ui.filterEdit->text() + ".*", QRegularExpression::CaseInsensitiveOption);
    ...
    for (int i = 0; i < m_items.size(); i++)
    {
        ...
        show &=
            ((m_items[i].second->childCount() == 0) || ui.filterEdit->text() == "" ||
             rx.match(m_items[i].second->text(5)).hasMatch());
    ...
    for (int i = 0; i < m_items.size(); i++)
    {
        ...
        if (m_items[i].second->parent() && !m_items[i].second->isHidden() &&
            !m_items[i].second->parent()->isHidden())
        {
    ...
    ui.groupBox_2->setTitle(
        QString("%1 (%2)")
            .arg(m_windowTitle)
            .arg(stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3]));

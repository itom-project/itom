...
void WidgetPropConsoleWrap::readSettings()
{
    ...
    if(flagStart == "NoFlag") index = 0;
    if(flagStart == "FlagText") index = 1;
    if(flagStart == "FlagBorder") index = 2;
    ui.comboFlagStart->setCurrentIndex(index);

    QString flagEnd = settings.value("WrapFlagEnd", "NoFlag").toString();
    if(flagEnd == "NoFlag") index = 0;
    if(flagEnd == "FlagText") index = 1;
    if(flagEnd == "FlagBorder") index = 2;
...
void WidgetPropConsoleWrap::writeSettings()
{
    ...
    int wrapMode;
    if(ui.radioWrapMode1->isChecked()) wrapMode = 0;
    if(ui.radioWrapMode2->isChecked()) wrapMode = 1;
    if(ui.radioWrapMode3->isChecked()) wrapMode = 2;
    ...
    int wrapIndentMode;
    if(ui.radioIndentMode1->isChecked()) wrapIndentMode = 0;
    if(ui.radioIndentMode2->isChecked()) wrapIndentMode = 1;
    if(ui.radioIndentMode3->isChecked()) wrapIndentMode = 2;

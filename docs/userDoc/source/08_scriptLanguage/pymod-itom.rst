...
is importable only in the embedded |python| interpreter in |itom|. This module includes interfaces for hardware and algorithm plugins of |itom| (TODO: add ref) as well as classes that wrap the
...
* class :py:class:`~itom.dataIO` is the class in order to access any plugin instance of type **dataIO** (cameras, grabbers, AD-converter...). The full reference can be found under :py:class:`~itom.dataIO`. More information about cameras and other dataIO devices can be found under TODO: add ref.
...
* **class actuator** is the class in order to access any plugin instance of type **actuator**, like motor stages... The full reference can be found under :py:class:`~itom.actuator`. More information about actuator devices can be found under TODO: add ref.

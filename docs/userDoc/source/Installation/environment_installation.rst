...
- OpenCV 2.3 (do not use version 2.2) 
...
1. Please consider that the whole installation process needs some hours. The longest step (compilation of Qt for Visual Studio 2010) can be run during the night. 
2. If you use a laptop, please make sure, that the energy management is properly set, such that the laptop's hard drive or operating system is not shutting down or going into real sleeping mode while executing the compilation process. 
3. If you already compiled parts of the project, read the hints for reversing these versions first.
4. Most of the required software packages can either be downloaded from the internet or be found in a sorted folder structure at the following Obelix server-address: 

.. code-block:: none

   \\Obelix\software\m\ITOM\Installationen
   
5. This document recommends to install certain software packages at given absolute file pathes. This is not necessary and you can easily replace the corresponding pathes by your individual structure. 
...
1. Visual-Studio: click View >> Property-Manager
2. For every project, the file itom_properties should be available

    - Double-Click on any of these files (changes may influence all projects) and click on tab User-defined macros
   
3. If the file is not available, you can add an existing property-file or create a new one by the toolbar above the property-manager in Visual Studio

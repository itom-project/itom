...
def create_package_file(root, master_package, subroot, py_files, opts, subs=None):
    ...
    if not opts.force and os.path.isfile(name):
        ...
        for py_file in py_files:
            ...
            text += write_directive(
                kind == "Package" and subroot or py_path, master_package
            )
            text += "\n"
        ...
        if root == path:
            ...
            if len(py_files) == 1:
                ...
                if check_for_code(os.path.join(root, "__init__.py")):
                    create_package_file(
                        root, package_name, subroot, py_files, opts=opts
                    )
                    toc.append(subroot)
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        usage="""usage: %(prog)s [options] <package path> [exclude paths, ...]

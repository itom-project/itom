...
public:
  ...
  /// SliderWidget forces the value to be within the
  ...
  /// SliderWidget stacked upon each other.
  ...
  void setSynchronizeSiblings(SliderWidget::SynchronizeSiblings options);
  ...
  /// False by default.
  /// Note: some sizing issues in the popup can happen if the SliderWidget
  /// has already parent. You might want to consider setting this property
  /// before setting a parent to SliderWidget.
  bool hasPopupSlider()const;
  void setPopupSlider(bool popup);

  ///
  /// Return the popup if SliderWidget hasPopupSlider() is true, 0 otherwise.
  /// It can be useful to control where the popup shows up relative to the
  /// spinbox the popup \sa PopupWidget::baseWidget.

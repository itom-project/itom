...
// -------------------------------------------------------------------------
TreeComboBoxPrivate::TreeComboBoxPrivate(TreeComboBox& object) : q_ptr(&object)
{
...
// -------------------------------------------------------------------------
TreeComboBox::TreeComboBox(QWidget* _parent) :
    Superclass(_parent), d_ptr(new TreeComboBoxPrivate(*this))
{
    ...
    connect(treeView, SIGNAL(collapsed(QModelIndex)), this, SLOT(resizePopup()));
    connect(treeView, SIGNAL(expanded(QModelIndex)), this, SLOT(resizePopup()));
}
...
bool TreeComboBox::eventFilter(QObject* object, QEvent* _event)
{
    ...
    case QEvent::MouseButtonRelease:
        ...
        if (this->view()->model()->hasChildren(index) && (index.flags() & Qt::ItemIsSelectable) &&
...
void TreeComboBox::showPopup()
{
    ...
    {
        header->setSectionHidden(i, d->VisibleModelColumn != -1 && i != d->VisibleModelColumn);
    }
...
void TreeComboBox::resizePopup()
{
    ...
    {
        listRect.setHeight(
            listRect.height() + style->pixelMetric(QStyle::PM_MenuVMargin, &opt, this) * 2);
    }
    ...
    // we need to activate the layout to make sure the min/maximum size are set when the widget was
    // not yet show
    container->layout()->activate();
    // takes account of the minimum/maximum size of the container
    listRect.setSize(
        listRect.size().expandedTo(container->minimumSize()).boundedTo(container->maximumSize()));
    ...
    if (usePopup)
    {
        ...
        const int height =
            !boundToScreen ? listRect.height() : qMin(listRect.height(), screen.height());
#ifdef Q_WS_S60

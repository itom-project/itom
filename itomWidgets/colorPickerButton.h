...
/// \ingroup Widgets
///
/// ColorPickerButton is a QPushButton that refers to a color. The color
...
/// for the QPushButton.
class ITOMWIDGETS_EXPORT ColorPickerButton : public QPushButton
{
  Q_OBJECT
...
public:
  ...
  explicit ColorPickerButton(const QString& text, QWidget* parent = 0 );

  /// The text will be shown on the button if
  /// displayColorName is false, otherwise the color name is shown.
  /// \sa setColor, QPushButton::setText
  explicit ColorPickerButton(const QColor& color, const QString & text, QWidget* parent = 0 );
...
public Q_SLOTS:
  ...
  /// If the CTK color dialog (\a UseColorDialog) is used, then the color
  /// name is also set if the user selects a named color.
  /// \sa ColorDialog, color, colorName
  void changeColor();
...
protected:
  ...
  QScopedPointer<ColorPickerButtonPrivate> d_ptr;
private :
  Q_DECLARE_PRIVATE(ColorPickerButton);
  Q_DISABLE_COPY(ColorPickerButton);
};

Q_DECLARE_OPERATORS_FOR_FLAGS(ColorPickerButton::ColorDialogOptions)

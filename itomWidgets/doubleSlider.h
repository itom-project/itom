...
/// \ingroup Widgets
/// DoubleSlider is a QSlider that controls doubles instead of integers.
/// DoubleSlider internally aggregates a QSlider
/// TODO: DoubleSlider tries to represent a double value with integers. It's
/// of course non-optimal and can lead to errors, it would be better if
/// DoubleSlider works like QDoubleSpinBox, where the value is a QVariant
/// and the conversion between double and integer is only done to convert
/// to/from screen coordinates.
/// \sa RangeSlider, DoubleRangeSlider, RangeWidget
class ITOMWIDGETS_EXPORT DoubleSlider : public QWidget
{
  Q_OBJECT
...
public:
  ...
  /// Constructors, builds a slider whose default values are the same as
  /// QSlider (vertical by default).
  explicit DoubleSlider(Qt::Orientation orient, QWidget* parent = 0);
  /// Destructor

...
void ITOMWIDGETS_EXPORT qListToSTLVector(const QStringList& list, std::vector<char*>& vector);

///
/// \ingroup Core
/// Convert a QStringList to a Vector of string
void ITOMWIDGETS_EXPORT qListToSTLVector(const QStringList& list, std::vector<std::string>& vector);

///
/// \ingroup Core
/// Convert a Vector of string to QStringList
void ITOMWIDGETS_EXPORT stlVectorToQList(const std::vector<std::string>& vector, QStringList& list);
...
QStringList ITOMWIDGETS_EXPORT nameFilterToExtensions(const QString& nameFilter);

///
/// \ingroup Core
/// Convert a nameFilter to a list of file extensions:
/// "Images (*.png *.jpg *.tiff)", "Text (*.txt)" -> "*.png", "*.jpg", "*.tiff", "*.txt"
QStringList ITOMWIDGETS_EXPORT nameFiltersToExtensions(const QStringList& nameFilters);

///
/// \ingroup Core
/// Convert a wildcar extension filter ("*.jpg") into a regular expression string
/// "*.jpg" -> ".*\\.jpg?$"
QString ITOMWIDGETS_EXPORT extensionToRegExp(const QString& extension);
...
int ITOMWIDGETS_EXPORT significantDecimals(double value, int defaultDecimals = -1);
...
int ITOMWIDGETS_EXPORT orderOfMagnitude(double value);
...
double ITOMWIDGETS_EXPORT closestPowerOfTen(double value);
...
/// \sa QDir::rmdir
bool ITOMWIDGETS_EXPORT removeDirRecursively(const QString & dirName);
...
/// \return <code>true</code> on success, <code>false</code> otherwise.
/// \sa QFile::copy
bool ITOMWIDGETS_EXPORT copyDirRecursively(const QString &srcPath, const QString &dstPath);

///
/// \ingroup Core
/// Convert Qt::HANDLE to string
/// \sa Qt::HANDLE
QString ITOMWIDGETS_EXPORT qtHandleToString(Qt::HANDLE handle);
...
qint64 ITOMWIDGETS_EXPORT msecsTo(const QDateTime& t1, const QDateTime& t2);

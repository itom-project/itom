...
namespace ito
{
    ...
    ito::RetVal formatDoubleWithUnit(QStringList scaleThisUnitsOnly, QString unitIn, double dVal, double &dValOut, QString &unitOut);
    //!< Function generates the auto save filename from the plugin name and the dll-folder
    ito::RetVal generateAutoSaveParamFile(QString plugInName, QFile &paramFile);
    ...
    ito::RetVal saveDOBJ2XML(ito::DataObject *dObjOut, QString folderFileName, bool onlyHeaderObjectFile = false, bool doubleAsBinary = false);

    //!< Import a dataObject from harddrive, saved in the ITO-XML-Format (.ido or .idh)
    ito::RetVal loadXML2DOBJ(ito::DataObject *dObjIn, QString folderFileName, bool onlyHeaderObjectFile = false, bool appendEnding = true);

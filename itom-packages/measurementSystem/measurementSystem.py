...
class MeasurementSystemBase():
    ...
    def __init__(self, name, probingSystem, instrument, username):
        ...
        if(not(isinstance(probingSystem, ProbingSystemType))):
            raise RuntimeError("probingSystem must be an instance of class 'ProbingSystemType'")
        if(not(isinstance(instrument, InstrumentType))):
            raise RuntimeError("instrument must be an instance of class 'InstrumentType'")
    ...
    def add25DMeasurementTags(self, dObj, axisScales, axisUnits, axisOffsets, axisDescriptions, valueUnit, valueDescription, measurementDate = None):
        '''
        ...
        if(isinstance(measurementDate, datetime)):
            dObj.setTag("measurementDate", measurementDate.strftime("%Y-%m-%dT%H:%M:%S.%f"))
    ...
    def getSystemCapabilities(self):
        ...
        ...
        '''
        raise RuntimeError("getSystemCapabilites has to be implemented by your measurement system")

...
class camToolbar(abstractObjInteractionToolBar):
    ...
    def getCamName(self, defaultCamName, valuename, textstring = "{} for Camera:"):
        ...
        '''
        [varname, check] = ui.getText("Variable input", textstring.format(valuename), defaultCamName)
    ...
    def getGrabbers(self, title, camname):
        ...
        for key in gvar:
            if type(globals()[key]) == itom.dataIO:
                ...
                if typ & 128:          # 0x01 = dataIO, # 0x80 = grabber --> only grabber allowed 
                    grabbers.append(key)
    ...
    def takeSnapshot(self, skipBox = False, defaultCamName = None, defaultVarName = None):
        ...
        dialogSnapshot = ui(itom.getAppPath()+"/itom-packages/camToolbar/snapshot.ui", ui.TYPEDIALOG, ui.BUTTONBAR_HORIZONTAL, dialogButtons = {"AcceptRole":"OK", "RejectRole":"Cancel"})
        ...
        if retdialog == 1:
            dataObj = dialogSnapshot.comboBoxdObj["currentText"] #.getProperty("comboBoxdObj", "currentText")
            #dataObj = dataObj[0]

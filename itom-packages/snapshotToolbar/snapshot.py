...
class Snapshot(ItomUi):
    def __init__(self,name):
        ...
        ItomUi.__init__(self,self.absPath,ui.TYPEWINDOW,)
        self.gui.setAttribute(55,True)
        
        
        self.direct=None
        self.dataTyp={"BMP":["gray","rgb","rgba","grayMarked",'falseColor','falseColorIR','hotIron'],"IDC":["single","stack"],"JPG":["gray","rgb","rgba","grayMarked",'falseColor','falseColorIR','hotIron'],"PGM":["gray"],"PNG":["gray","rgb","rgba","grayMarked",'falseColor','falseColorIR','hotIron'],"PPM":["gray","rgb","rgba","grayMarked",'falseColor','falseColorIR','hotIron'],"RAS":["gray","rgb","rgba","grayMarked",'falseColor','falseColorIR','hotIron'],"Tiff":["gray","rgb","rgba","grayMarked",'falseColor','falseColorIR','hotIron']}
    ...
    def callPathDialog(self):
        if self.direct is None:
            ...
            options = 1|2
            self.direct = itom.ui.getExistingDirectory('save images',self.direct, options)
        ...
        if self.gui.checkMulti["checked"]:
            if not self.gui.checkTimer["checked"]:
                try:
                    ...
                    typ=firstpic.dtype
                    dat=dataObject([self.gui.spinMulti["value"],self.selectedDevice.getParam("sizey"),self.selectedDevice.getParam("sizex")],dtype=typ)
                    dat[0,:,:]=firstpic
                    i=1
                    while i < dat.shape[0]:
                        self.selectedDevice.acquire()
    ...
    def timerPic(self):
        ...
        try:
            if self.cnt==0:
                ...
                self.selectedDevice.copyVal(firstpic)
                typ=firstpic.dtype
                self.dat=dataObject([self.gui.spinMulti["value"],self.selectedDevice.getParam("sizey"),self.selectedDevice.getParam("sizex")],dtype=typ)
                self.dat[0,:,:]=firstpic
                self.cnt=self.cnt+1
    ...
    def saveFiles(self,file):
        ...
        if filestr=="saveIDC":
            ...
            while self.checkName(filename):
                ...
                filename="pic_%03i.%s" %(idx,self.Typ)
            try:
                filter(filestr,file,filename,self.dataTyp[self.Typ][self.gui.comboColor["currentIndex"]])
            except Exception:
                ui.msgCritical("Error",'Not able to save files. Maybe the selcted color palette does not fit to data type')

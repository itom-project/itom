...
def polyfitweighted2(koX, koY, koZ, n, margin = 0.2, w = None):
    ...
    koX = koX.reshape([koX.size,1]) #These are row Vectors in difference to ployval2 function
    ...
    z=koZ.copy()
    ma = np.isfinite(z)
    x = x[ma]               #Z-Werte werden gemessen, und koennen NaN oder infinite sein, solche Werte werden ausgefiltert
    y = y[ma]
    pw = pw[ma]
    z = z[ma]
    ...
    V=np.zeros((pts,np.int_((n+1)*(n+2)/2))) #Matrix wird angelegt/Reserviert
    V[:,0] = pw
    ordercolumn=1
    ...
    try:
        ...
        print("Hey das geht nicht! QR- Zerlegung failure, badly conditioned inputmatrix.You probably mismatched the input Vectors.")
        return None

    if R.shape[1] > R.shape[0]:
        print('polyfitweighted2:PolyNotUnique Polynomial is not unique; degree >= number of data points.')
    #elif np.linalg.cond(R) > 1.0e10:
    elif nputils.condest(R) > 1.0e10: #in Mathlab war das mal 'condest(R)' Warnung bezueglich Unterbestimmung, wenn gleiche Punkte in der Matrix auftauchen
        print("""  polyfitweighted2:RepeatedPointsOrRescale Polynomial is badly conditioned. 

...
class surfaceAnalysisTools(abstractObjInteractionToolBar):
    ...
    def __init__(self, myName, appendButtons = True, appendMenu = True, defPrecision = 3, defaultVar = 'dObj'):
        ...
        if appendMenu == True:
            ...
            addMenu(BUTTON, hashName + "/alignObject/polyfit", "Fit polynom", self.planefit)
            addMenu(BUTTON, hashName + "/alignObject/planefit", "Fit line or plane", self.polyfit)
            addMenu(MENU, hashName + "/roughness", "roughness calculation")
            addMenu(BUTTON, hashName + "/roughness/Rz_Sz", "Calculate Rz or Sz", self.calcRzSz)
            addMenu(BUTTON, hashName + "/roughness/Ra_Sa", "Calculate Ra or Sa", self.calcRaSa)
            addMenu(BUTTON, hashName + "/roughness/Rq_Sq", "Calculate Rq or Sq", self.calcRqSq)
            addMenu(BUTTON, hashName + "/roughness/Sz_10P", "Calculate former Sz", self.calcSz10P)
    ...
    def calcRzSz(self, skipBox = False, defaultVarName = None):
        ...
        [varname, obtype, check]= self.getVarNameDialog("Rz/Sz", varname, globals(), 'ND')
        ...
        if check == True:
            ...
            try:
                ...
                if line == True:
                    if skipBox == False:
                        [scaledResult, scaledUnit] = scaleDoubleUnit(getDefaultScaleableUnits(), result[0], valueUnit)
                        ui.msgInformation("Roughness", "Rz = {:.4f} {}\nSkipped {} Pixel".format(scaledResult, scaledUnit,result[1]), ui.MsgBoxOk)
                else:
                    if skipBox == False:
                        [scaledResult0, scaledUnit0] = scaleDoubleUnit(getDefaultScaleableUnits(), result[0], valueUnit)
                        [scaledResult1, scaledUnit1] = scaleDoubleUnit(getDefaultScaleableUnits(), result[1], valueUnit)
                        [scaledResult2, scaledUnit2] = scaleDoubleUnit(getDefaultScaleableUnits(), result[2], valueUnit)
                        ui.msgInformation("Roughness", "Sv = {:.4f} {}\nSp = {:.4f} {}\nSz = {:.4f} {}".format(scaledResult0, scaledUnit0, scaledResult1, scaledUnit1, scaledResult2, scaledUnit2), ui.MsgBoxOk)
            except:
                if skipBox == False:
    ...
    def calcSz10P(self, skipBox = False, defaultVarName = None):
        ...
        # Get the varaiblename as string
        #[varname, check]= self.__getVarNameText(varname, "Rz/Sz")
        [varname, obtype, check]= self.getVarNameDialog("TenPointHeigth (former Sz)", varname, globals(), 'ND')
        ...
        if check == True:
            ...
            try:
                ...
                if skipBox == False:
                    [scaledResult, scaledUnit] = scaleDoubleUnit(getDefaultScaleableUnits(), result[0], valueUnit)
                    ui.msgInformation("Roughness", "Sz 10P = {:.4f} {}\nSkipped {} Pixel in y\nSkipped {} Pixel in x".format(scaledResult, scaledUnit,result[1],result[2]), ui.MsgBoxOk)
            except:
                if skipBox == False:
                    ui.msgCritical("Roughness", "Calculation error", ui.MsgBoxOk)
                check = False
    ...
    def calcRaSa(self, skipBox = False, defaultVarName = None):
        ...
        [varname, obtype, check]= self.getVarNameDialog("Ra/Sa", varname, globals(), 'ND')
        ...
        if check == True:
            ...
            try:
                ...
                valueUnit = eval("{}.valueUnit".format(varname));
                if skipBox == False:
                    [scaledResult, scaledUnit] = scaleDoubleUnit(getDefaultScaleableUnits(), result, valueUnit)
                    ui.msgInformation("Roughness", printScript.format(scaledResult, scaledUnit), ui.MsgBoxOk)
            except:
                if skipBox == False:
                    ui.msgCritical("Roughness", "Calculation error", ui.MsgBoxOk)
                check = False
    ...
    def calcRqSq(self, skipBox = False, defaultVarName = None):
        ...
        [varname, obtype, check]= self.getVarNameDialog("Rq/Sq", defaultVar, globals(), 'ND')
    ...
    def planefit(self, skipBox = False, defaultVarName = None):
        ...
        [varname, obtype, check]= self.getVarNameDialog("Plane-Fit", defaultVar, globals(), 'ND')
    ...
    def polyfit(self, skipBox = False, defaultVarName = None):
        ...
        [varname, obtype, check]= self.getVarNameDialog("Polynome-Fit", defaultVar, globals(), 'ND')
        ...
        [self.defXGradPoly, check] = ui.getInt("Polyfit", "Order in x", self.defXGradPoly)
        ...
        if check == True:
            ...
            try:
                if line == True:
                    ...
                    script = "globals()[\"{}\"] = {}.astype('float64') - tempObjVar".format(varname, varname)
                    exec(script, globals(), {"tempObjVar" : tempObjVar})
                else:
                    [self.defYGradPoly, check] = ui.getInt("Polyfit", "Grade in y", self.defYGradPoly)
                    script = "filter(\"fitPolynom2D\",{}, {}, {}, {})\n"
                    result = eval(script.format(varname, "tempObjVar", str(self.defXGradPoly), str(self.defYGradPoly)))
                    script = "globals()[\"{}\"] = {}.astype('float64') - tempObjVar".format(varname, varname)
                    exec(script)
            except:
                if skipBox == False:

...
def rename_references(app, what, name, obj, options, lines, reference_offset=[0]):
    # replace reference numbers so that there are no duplicates
    references = []
    for line in lines:
        line = line.strip()
        m = re.match(
            sixu("^.. \\[(%s)\\]") % app.config.numpydoc_citation_re, line, re.I
        )
        if m:
...
def match_items(lines, content_old):
    ...
    keep track of where each line came from. The managing does many operations

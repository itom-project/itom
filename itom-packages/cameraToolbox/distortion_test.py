...
def distortionTest():
    ...
    points_d = distortion.getPointGridDistorted([rows,cols], pitch, center, rotation, k1, k2, k3, '3d')

    x0 = distortion.guessInitialParameters(points_d, rows, cols, withDistortion = False, withRotation = True)
    ...
    itom.plot(distortion.drawPointGrid(points_u, rows, cols), properties={"title":"undistorted image"})
    itom.plot(distortion.drawPointGrid(points_d, rows, cols), properties={"title":"distorted image"})
    itom.plot(distortion.drawPointGrid(points_corrected, rows, cols), properties={"title":"corrected image"})
    itom.plot(distortion.createDistortionMap(coeffs_fine, points_d, rows, cols), properties={"title":"Distortion", \
